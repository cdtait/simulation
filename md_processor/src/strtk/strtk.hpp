...
inline std::size_t for_each_token(const std::string &buffer,
		Tokenizer &tokenizer, Function function) {
...
template<typename Function>
inline std::size_t for_each_line_n(std::istream &stream, const std::size_t &n,
		Function function, const std::size_t &buffer_size = one_kilobyte) {
	std::string buffer;
	buffer.reserve(buffer_size);
	std::size_t line_count = 0;
	while (std::getline(stream, buffer)) {
...
inline std::size_t for_each_line_n(const std::string &file_name,
		const std::size_t &n, Function function,
		const std::size_t &buffer_size = one_kilobyte) {
	std::ifstream stream(file_name.c_str());
	if (stream)
...
template<typename T>
inline bool read_line_as_value(std::istream &stream, T &t,
		const std::size_t &buffer_size = one_kilobyte) {
	std::string buffer;
	buffer.reserve(buffer_size);
...
struct is_valid_iterator {
	typedef typename details::enable_if<
			details::supported_iterator_type<T>::value, T>::type type;
};
...
inline bool string_to_type_converter(const std::string &s, T &t) {
	return string_to_type_converter<const char*, T>(s.data(),
			s.data() + s.size(), t);
}
...
inline std::size_t load_from_text_file(std::istream &stream,
		Sequence<T, Allocator> &sequence, const std::size_t &buffer_size =
				one_kilobyte) {
	if (!stream)
		return 0;
	std::string buffer;
...
inline std::size_t load_from_text_file(std::istream &stream,
		std::queue<T, Container> &queue, const std::size_t &buffer_size =
				one_kilobyte) {
	if (!stream)
		return 0;
	std::string buffer;
...
template<typename T, typename Container>
inline std::size_t load_from_text_file(std::istream &stream,
		std::stack<T, Container> &stack, const std::size_t &buffer_size =
				one_kilobyte) {
	if (!stream)
		return 0;
	std::string buffer;
...
inline std::size_t write_to_text_file(std::ostream &stream,
		const Sequence<T, Allocator> &sequence, const std::string &delimiter =
				"") {
	if (!stream)
		return 0;
...
inline std::size_t write_to_text_file(std::ostream &stream,
	...
	std::size_t count = 0;
	typename std::set<T, Comparator, Allocator>::const_iterator itr =
			set.begin();
...
inline void copy_if(Predicate predicate, const InputIterator begin,
		const InputIterator end, OutputIterator out) {
...
template<typename Predicate, typename InputIterator, typename OutputIterator>
inline InputIterator copy_while(Predicate predicate, const InputIterator begin,
		const InputIterator end, OutputIterator out) {
	InputIterator itr = begin;
	while (end != itr) {
		if (!predicate(*itr))
...
template<typename Predicate, typename InputIterator, typename OutputIterator>
inline InputIterator copy_until(Predicate predicate, const InputIterator begin,
		const InputIterator end, OutputIterator out) {
	InputIterator itr = begin;
	while (end != itr) {
		if (predicate(*itr))
...
inline bool range_only_contains(Predicate predicate, const InputIterator begin,
...
public:
	...
	adapter(const std::pair<T*, T*> &r) :
			begin_(r.first), end_(r.second) {
	}

	adapter(T *const begin, const std::size_t length) :
			begin_(begin), end_(begin_ + length) {
	}
...
private:
	...
	single_delimiter_predicate<T>& operator=(
			const single_delimiter_predicate<T>&);
	const T delimiter_;
};
...
public:
	...
	multiple_delimiter_predicate(const T *d_begin, const T *d_end) :
			length_(std::distance(d_begin, d_end)), delimiter_(
					(length_ <= sbo_buffer_size) ? sbo_buffer : new T[length_]), delimiter_end_(
					delimiter_ + length_) {
		std::copy(d_begin, d_end, delimiter_);
	}

	multiple_delimiter_predicate(const T d[], const std::size_t &length) :
			length_(length), delimiter_(
					(length_ <= sbo_buffer_size) ? sbo_buffer : new T[length_]), delimiter_end_(
					delimiter_ + length_) {
	...
	multiple_delimiter_predicate(const Iterator begin, const Iterator end) :
			length_(std::distance(begin, end)), delimiter_(
					(length_ <= sbo_buffer_size) ? sbo_buffer : new T[length_]), delimiter_end_(
					delimiter_ + length_) {
	...
	multiple_delimiter_predicate(const range::adapter<Type> &r) :
			length_(std::distance(r.begin(), r.end())), delimiter_(
					(length_ <= sbo_buffer_size) ? sbo_buffer : new T[length_]), delimiter_end_(
					delimiter_ + length_) {
		//static_assert(T == std::iterator_traits<Iterator>::value_type);
...
private:
	...
	multiple_delimiter_predicate& operator=(
			const multiple_delimiter_predicate<T> &mdp);
...
public:
	...
	template<typename Iterator>
	multiple_char_delimiter_predicate(const Iterator begin,
			const Iterator end) {
...
private:
	...
	template<typename Iterator>
	inline void setup_delimiter_table(const Iterator begin,
			const Iterator end) {
...
struct index_remover_impl {
	...
	index_remover_impl(const sequence_t &sequence) :
			itr_(sequence.begin()), end_(sequence.end()), current_index_(0), check_(
					true) {
	}
...
inline std::size_t remove_inplace(Predicate predicate, Iterator begin,
		Iterator end) {
	Iterator itr1 = begin;
	Iterator itr2 = begin;
...
template<typename Predicate, typename T, typename Allocator, template<typename,
...
inline void remove_consecutives_inplace(std::string &s) {
	std::size_t removal_count = remove_consecutives_inplace(
			const_cast<char*>(s.data()),
			const_cast<char*>(s.data() + s.size()));
...
template<typename Iterator, typename Predicate>
inline std::size_t remove_trailing(Predicate predicate, Iterator begin,
		Iterator end) {
...
template<typename Iterator, typename Predicate>
inline std::size_t remove_leading(Predicate predicate, Iterator begin,
		Iterator end) {
	const std::size_t length = std::distance(begin, end);
	if (0 == length)
		return 0;
...
template<typename Comparator, typename Allocator>
void remove_empty_strings(
		std::multiset<std::string, Comparator, Allocator> &set) {
	struct is_empty {
		static inline bool check(const std::string &s) {
			return s.empty();
		}
...
inline void replace(const std::string::value_type &c0,
		const std::string::value_type &c1, std::string &s) {
...
inline std::size_t replace_pattern(const InputIterator s_begin,
	...
	if ((0 == p_size)
			|| ((p_size == r_size) && std::equal(p_begin, p_end, r_begin))) {
...
inline void remove_pattern(const std::string &s, const std::string &p,
		std::string &n) {
...
inline bool match(const std::string &wild_card, const std::string &str) {
	/*
	 * : Zero or more match
	 ? : Zero or one match
	 */
...
inline bool imatch(const std::string &s1, const std::string &s2) {
	return imatch(s1.data(), s1.data() + s1.size(), s2.data(),
			s2.data() + s2.size());
}
...
inline std::size_t find_all(const Iterator pattern_begin,
		const Iterator pattern_end, const Iterator begin, const Iterator end,
		OutputIterator out) {
	Iterator itr = begin;
	const std::size_t pattern_length = std::distance(pattern_begin,
			pattern_end);
...
template<typename Iterator, typename Range, typename Allocator, template<
...
inline std::size_t ifind(const std::string &pattern, const std::string &data) {
	...
	const char *result_itr = std::search(data.data(), data.data() + data.size(),
			pattern.data(), pattern.data() + pattern.size(), imatch_char);
...
inline std::size_t ifind_all(const Iterator pattern_begin,
	...
	while (end
			!= (itr = std::search(itr, end, pattern_begin, pattern_end,
					imatch_char))) {
		(*out) = std::make_pair(itr, itr + pattern_length);
		itr += pattern_length;
		++out;
...
inline std::size_t find_all(const std::string &pattern, const std::string &data,
		OutputIterator out) {
	return find_all(pattern.data(), pattern.data() + pattern.size(),
			data.data(), data.data() + data.size(), out);
}
...
inline std::size_t ifind_all(const std::string &pattern,
		const std::string &data, OutputIterator out) {
...
inline bool begins_with(const std::string &pattern, const std::string &data) {
	if (pattern.size() <= data.size()) {
		return begins_with(pattern.data(), pattern.data() + pattern.size(),
...
inline bool ibegins_with(const std::string &pattern, const std::string &data) {
	if (pattern.size() <= data.size()) {
		return ibegins_with(pattern.data(), pattern.data() + pattern.size(),
				data.data(), data.data() + data.size());
	} else
		return false;
}
...
inline bool ends_with(const InputIterator pattern_begin,
	...
	if (pattern_length <= data_length) {
		return std::equal(pattern_begin, pattern_end,
...
inline bool ends_with(const std::string &pattern, const std::string &data) {
	if (pattern.size() <= data.size()) {
		return ends_with(pattern.data(), pattern.data() + pattern.size(),
				data.data(), data.data() + data.size());
	} else
		return false;
}
...
inline bool iends_with(const InputIterator pattern_begin,
	...
	if (pattern_length <= data_length) {
		return std::equal(pattern_begin, pattern_end,
				begin + (data_length - pattern_length), imatch_char);
	} else
		return false;
}
...
inline bool iends_with(const std::string &pattern, const std::string &data) {
	if (pattern.size() <= data.size()) {
		return iends_with(pattern.data(), pattern.data() + pattern.size(),
				data.data(), data.data() + data.size());
	} else
		return false;
}
...
inline std::size_t index_of(const std::string &pattern,
	...
	if (pattern.empty())
		...
		return std::string::npos;
	const char *itr = std::search(data.data(), data.data() + data.size(),
			pattern.data(), pattern.data() + pattern.size());
	return ((data.data() + data.size()) == itr) ?
			std::string::npos : std::distance(data.data(), itr);
}
...
private:
	...
	template<typename Iterartor, typename Predicate, typename T = std::pair<
			Iterator, Iterator> >
	...
	public:
		...
		explicit inline tokenizer_iterator(const iterator begin,
				const iterator end, const Predicate &predicate,
				const tokenize_options::type tokenize_option =
						tokenize_options::default_mode) :
				predicate_(predicate), end_(end), range_(begin, begin), current_token_(
						end, end), compress_delimiters_(
						tokenize_options::perform_compress_delimiters(
								tokenize_option)), include_1st_delimiter_(
						tokenize_options::perform_include_1st_delimiter(
								tokenize_option)), include_all_delimiters_(
						tokenize_options::perform_include_all_delimiters(
								tokenize_option)), include_delimiters_(
						include_1st_delimiter_ || include_all_delimiters_), last_token_done_(
						false) {
			if (end != begin) {
				this->operator++();
			}
...
public:
	...
	inline tokenizer(const Iterator begin, const Iterator end,
			const DelimiterPredicate &predicate,
			const tokenize_options::type tokenize_options =
					tokenize_options::default_mode) :
			tokenize_options_(tokenize_options), predicate_(predicate), begin_(
					begin), end_(end), begin_itr_(begin_, end_, predicate_,
					tokenize_options_), end_itr_(end_, end_, predicate_,
					tokenize_options_) {
	}

	inline tokenizer(const std::string &s, const DelimiterPredicate &predicate,
			const tokenize_options::type tokenize_options =
					tokenize_options::default_mode) :
			tokenize_options_(tokenize_options), predicate_(predicate), begin_(
					s.data()), end_(s.data() + s.size()), begin_itr_(begin_,
					end_, predicate_, tokenize_options_), end_itr_(end_, end_,
					predicate_, tokenize_options_) {
	}
...
template<typename DelimiterPredicate = single_delimiter_predicate<
...
struct tokenizer {
	...
	typedef strtk::tokenizer<string_iterator_type,
			multiple_char_delimiter_predicate> md_type;
...
class range_to_type_back_inserter_iterator: public std::iterator<
...
public:
	...
	range_to_type_back_inserter_iterator(
			const range_to_type_back_inserter_iterator &it) :
...
template<typename Sequence>
inline range_to_type_back_inserter_iterator<Sequence> range_to_type_back_inserter(
		Sequence &sequence) {
	return (range_to_type_back_inserter_iterator<Sequence>(sequence));
}

template<typename Set>
class range_to_type_inserter_iterator: public std::iterator<
		std::output_iterator_tag, void, void, void, void> {
public:
	...
	inline range_to_type_inserter_iterator& operator=(
			const range_to_type_inserter_iterator &it) {
		if (this != &it) {
			this->set_ = it.set_;
		}
		return (*this);
...
class range_to_type_push_inserter_iterator: public std::iterator<
		std::output_iterator_tag, void, void, void, void> {
public:
	...
	range_to_type_push_inserter_iterator(
			const range_to_type_push_inserter_iterator &it) :
...
template<typename Container>
inline range_to_type_push_inserter_iterator<Container> range_to_type_push_inserter(
		Container &container) {
	return (range_to_type_push_inserter_iterator<Container>(container));
}

template<typename Sequence>
class back_inserter_with_valuetype_iterator: public std::iterator<
		std::output_iterator_tag, typename Sequence::value_type, void, void,
		void> {
public:
	...
	back_inserter_with_valuetype_iterator(
			const back_inserter_with_valuetype_iterator &it) :
			sequence_(it.sequence_) {
	}
	...
	inline back_inserter_with_valuetype_iterator& operator=(
			const typename Sequence::value_type &v) {
...
template<typename Sequence>
inline back_inserter_with_valuetype_iterator<Sequence> back_inserter_with_valuetype(
		Sequence &sequence_) {
	return (back_inserter_with_valuetype_iterator<Sequence>(sequence_));
}

template<typename Set>
class inserter_with_valuetype_iterator: public std::iterator<
		std::output_iterator_tag, typename Set::value_type, void, void, void> {
public:
	...
	inserter_with_valuetype_iterator(
			const inserter_with_valuetype_iterator &itr) :
			set_(itr.set_) {
	}
...
public:
	...
	inline push_inserter_iterator& operator=(
			const push_inserter_iterator &itr) {
		if (this != &itr) {
...
public:
	...
	explicit inline range_to_ptr_type_iterator(T *pointer,
			...
			pointer_(pointer), insert_count_(insert_count) {
	}
	...
	inline range_to_ptr_type_iterator& operator=(
			const range_to_ptr_type_iterator &it) {
		if (this != &it) {
...
template<typename T>
class counting_back_inserter_iterator: public std::iterator<
...
public:
	...
	inline counting_back_inserter_iterator& operator=(
			const counting_back_inserter_iterator &itr) {
		if (this != &itr) {
...
template<typename Function>
class functional_inserter_iterator: public std::iterator<
		std::output_iterator_tag, void, void, void, void> {
public:
	...
	inline functional_inserter_iterator& operator=(
			const functional_inserter_iterator &it) {
		if (this != &it) {
...
namespace details {
template<typename DelimiterPredicate, typename Iterator, typename OutputIterator>
inline std::size_t split_compress_delimiters(
		const DelimiterPredicate &delimiter, const Iterator begin,
		const Iterator end, OutputIterator out) {
	std::size_t token_count = 0;
...
inline std::size_t split(const DelimiterPredicate &delimiter,
		const Iterator begin, const Iterator end, OutputIterator out,
...
template<typename DelimiterPredicate, typename Iterator, typename OutputIterator>
inline std::size_t split(const DelimiterPredicate &delimiter,
		const std::pair<Iterator, Iterator> &range, OutputIterator out,
		const split_options::type split_option = split_options::default_mode) {
	return split(delimiter, range.first, range.second, out, split_option);
}
...
inline std::size_t split(const char *delimiters,
	...
	if (1 == details::strnlength(delimiters, 256))
		return split(
				single_delimiter_predicate<std::string::value_type>(
						delimiters[0]), range.first, range.second, out,
				split_option);
	else
		return split(multiple_char_delimiter_predicate(delimiters), range.first,
				range.second, out, split_option);
}
...
template<typename OutputIterator>
inline std::size_t split(const char *delimiters, const std::string &str,
		OutputIterator out, const split_options::type &split_option =
				split_options::default_mode) {
	if (1 == details::strnlength(delimiters, 256))
		return split(
				single_delimiter_predicate<std::string::value_type>(
		...
		return split(multiple_char_delimiter_predicate(delimiters), str.data(),
				str.data() + str.size(), out, split_option);
}

template<typename OutputIterator>
inline std::size_t split(const std::string &delimiters, const std::string &str,
		OutputIterator out, const split_options::type &split_option =
				split_options::default_mode) {
	if (1 == delimiters.size())
		return split(
...
inline std::size_t split(const std::string::value_type delimiter,
		const std::string &str, OutputIterator out,
		const split_options::type &split_option = split_options::default_mode) {
	return split(single_delimiter_predicate<std::string::value_type>(delimiter),
			str.data(), str.data() + str.size(), out, split_option);
}
...
inline std::size_t split(const char *delimiters, const std::string &str,
	...
	if (1 == details::strnlength(delimiters, 256))
		return split(
				single_delimiter_predicate<std::string::value_type>(
						delimiters[0]), str.data(), str.data() + str.size(),
				std::back_inserter(sequence), split_option);
	else
		return split(multiple_char_delimiter_predicate(delimiters), str.data(),
				str.data() + str.size(), std::back_inserter(sequence),
				split_option);
}
...
inline std::size_t split(const DelimiterPredicate &delimiter,
		const std::string &str, OutputIterator out,
		const split_options::type &split_option = split_options::default_mode) {
	return split(delimiter, str.data(), str.data() + str.size(), out,
			split_option);
}
...
inline std::size_t split_n(const DelimiterPredicate &delimiter,
		const Iterator begin, const Iterator end,
		const std::size_t &token_count, OutputIterator out,
		const split_options::type &split_option = split_options::default_mode) {
	if (0 == token_count)
		return 0;
	if (begin == end)
...
template<typename OutputIterator>
inline std::size_t split_n(const char *delimiters, const std::string &str,
		const std::size_t &token_count, OutputIterator out,
		const split_options::type &split_option = split_options::default_mode) {
	return split_n(multiple_char_delimiter_predicate(delimiters), str.data(),
			str.data() + str.size(), token_count, out, split_option);
}

template<typename OutputIterator>
inline std::size_t split_n(const std::string &delimiters,
		const std::string &str, const std::size_t &token_count,
		OutputIterator out, const split_options::type &split_option =
				split_options::default_mode) {
	if (1 == delimiters.size())
		return split_n(
				single_delimiter_predicate<std::string::value_type>(
						delimiters[0]), str.data(), str.data() + str.size(),
				token_count, out, split_option);
	else
		return split_n(multiple_char_delimiter_predicate(delimiters),
				str.data(), str.data() + str.size(), token_count, out,
				split_option);
}
...
inline std::size_t split_n(const std::string &delimiters,
	...
	if (1 == delimiters.size())
		return split_n(
				single_delimiter_predicate<std::string::value_type>(
						delimiters[0]), begin, end, token_count, out,
				split_option);
	else
		return split_n(multiple_char_delimiter_predicate(delimiters), begin,
				end, token_count, out, split_option);
}
...
inline std::size_t split_n(const std::string::value_type delimiter,
	...
	return split_n(
			...
			str.data(), str.data() + str.size(), token_count, out, split_option);
}
...
inline std::size_t split_n(const DelimiterPredicate &delimiter,
		...
		OutputIterator out, const split_options::type &split_option =
				split_options::default_mode) {
	return split_n(delimiter, str.data(), str.data() + str.size(), token_count,
			out, split_option);
}

#ifdef strtk_enable_regex

static const std::string uri_expression(
		"((https?|ftp)\\://((\\[?(\\d{1,3}\\.){3}\\d{1,3}\\]?)|(([-a-zA-Z0-9]+\\.)+[a-zA-Z]{2,4}))(\\:\\d+)?(/[-a-zA-Z0-9._?,+&amp;%$#=~\\\\]+)*/?)");
static const std::string email_expression(
		"([\\w\\-\\.]+)@((\\[([0-9]{1,3}\\.){3}[0-9]{1,3}\\])|(([\\w\\-]+\\.)+)([a-zA-Z]{2,4}))");
static const std::string ip_expression(
		"(([0-2]*[0-9]+[0-9]+)\\.([0-2]*[0-9]+[0-9]+)\\.([0-2]*[0-9]+[0-9]+)\\.([0-2]*[0-9]+[0-9]+))");
static const std::string ieee754_expression(
...
inline std::size_t split_regex(const boost::regex &delimiter_expression,
		const InputIterator begin, const InputIterator end, OutputIterator out,
...
inline std::size_t split_regex(const std::string &delimiter_expression,
	...
	return split_regex(regex_expression, begin, end, out, mode);
}

template<typename OutputIterator>
inline std::size_t split_regex(const std::string &delimiter_expression,
		const std::string &text, OutputIterator out,
		const regex_match_mode::type mode = regex_match_mode::match_all) {
...
inline std::size_t split_regex_n(const std::string &delimiter_expression,
		...
		const regex_match_mode::type mode = regex_match_mode::match_all) {
	const boost::regex regex_expression(delimiter_expression);
	return split_regex_n(regex_expression, begin, end, token_count, out, mode);
}

template<typename OutputIterator>
inline std::size_t split_regex_n(const std::string &delimiter_expression,
		const std::string &text, const std::size_t &token_count,
		OutputIterator out, const regex_match_mode::type mode =
				regex_match_mode::match_all) {
	return split_regex_n(delimiter_expression, text.begin(), text.end(),
			token_count, out, mode);
}
...
public:
	...
	offset_predicate(const int offset_list[], const bool rotate = false) :
			rotate_(rotate), current_index_(0) {
...
inline offset_predicate<12> offsets(const int &v1, const int &v2, const int &v3,
		...
		const int &v12, const bool &rotate = false) {
	const int offset_list[12] = { v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11,
			v12 };
...
inline offset_predicate<10> offsets(const int &v1, const int &v2, const int &v3,
		const int &v4, const int &v5, const int &v6, const int &v7,
		const int &v8, const int &v9, const int &v10,
		const bool &rotate = false) {
...
inline offset_predicate<9> offsets(const int &v1, const int &v2, const int &v3,
		const int &v4, const int &v5, const int &v6, const int &v7,
		const int &v8, const int &v9, const bool &rotate = false) {
...
inline offset_predicate<8> offsets(const int &v1, const int &v2, const int &v3,
		const int &v4, const int &v5, const int &v6, const int &v7,
		const int &v8, const bool &rotate = false) {
...
inline offset_predicate<1> offsets(const int &v1, const bool &rotate = false) {
	const int offset_list[1] = { v1 };
	return offset_predicate<1>(offset_list, rotate);
}

template<typename OffsetPredicate, typename InputIterator,
		typename OutputIterator>
inline std::size_t offset_splitter(const InputIterator begin,
		const InputIterator end, const OffsetPredicate &offset,
		OutputIterator out) {
	std::size_t length = 0;
...
template<typename OffsetPredicate, typename OutputIterator>
inline std::size_t offset_splitter(const std::string &str,
		const OffsetPredicate &offset, OutputIterator out) {
	return offset_splitter(str.data(), str.data() + str.size(), offset, out);
}

template<typename InputIterator, typename Predicate, typename OutputPair>
inline bool split_pair(const InputIterator begin, const InputIterator end,
		const Predicate &delimiter, OutputPair &v1, OutputPair &v2) {
	if (0 == std::distance(begin, end))
		return false;
...
inline bool split_pair(const std::string::value_type delimiter,
		const std::string &str, std::pair<const char*, const char*> &v1,
		std::pair<const char*, const char*> &v2) {
	return split_pair(str.data(), str.data() + str.size(),
			single_delimiter_predicate<std::string::value_type>(delimiter), v1,
			v2);
}
...
inline bool split_pair(const DelimiterPredicate &delimiter,
		const std::string &str, std::pair<const char*, const char*> &v1,
		std::pair<const char*, const char*> &v2) {
	return split_pair(str.data(), str.data() + str.size(), delimiter, v1, v2);
}

template<typename Function>
inline std::size_t for_each_token(const std::string &buffer,
		const std::string &delimiters, Function function) {
	return split(delimiters, buffer,
			strtk::functional_inserter<Function>(function));
}

template<typename Function>
inline std::size_t for_each_token(const std::string &buffer,
		const char *delimiters, Function function) {
	return split(delimiters, buffer,
			strtk::functional_inserter<Function>(function));
}
...
inline void min_max_of_cont(const Sequence<T, Allocator> &sequence,
	...
	min_max_of_range(sequence.begin(), sequence.end(), min_value, max_value);
}
...
template<typename T, typename Comparator, typename Allocator>
inline void min_max_of_cont(
		const std::multiset<T, Comparator, Allocator> &multiset, T &min_value,
		T &max_value) {
...
inline const unsigned char* first_non_repeated_char(const unsigned char *begin,
	...
	return const_cast<const unsigned char*>(reinterpret_cast<unsigned char*>(const_cast<char*>(first_non_repeated_char(
			b, e))));
}
...
inline void convert_bin_to_hex(const unsigned char *begin,
	...
	static const unsigned short hex_lut[] = { 0x3030, 0x3130, 0x3230, 0x3330,
			0x3430, 0x3530, 0x3630, 0x3730, 0x3830, 0x3930, 0x4130, 0x4230,
			0x4330, 0x4430, 0x4530, 0x4630, 0x3031, 0x3131, 0x3231, 0x3331,
			0x3431, 0x3531, 0x3631, 0x3731, 0x3831, 0x3931, 0x4131, 0x4231,
			0x4331, 0x4431, 0x4531, 0x4631, 0x3032, 0x3132, 0x3232, 0x3332,
			0x3432, 0x3532, 0x3632, 0x3732, 0x3832, 0x3932, 0x4132, 0x4232,
			0x4332, 0x4432, 0x4532, 0x4632, 0x3033, 0x3133, 0x3233, 0x3333,
			0x3433, 0x3533, 0x3633, 0x3733, 0x3833, 0x3933, 0x4133, 0x4233,
			0x4333, 0x4433, 0x4533, 0x4633, 0x3034, 0x3134, 0x3234, 0x3334,
			0x3434, 0x3534, 0x3634, 0x3734, 0x3834, 0x3934, 0x4134, 0x4234,
			0x4334, 0x4434, 0x4534, 0x4634, 0x3035, 0x3135, 0x3235, 0x3335,
			0x3435, 0x3535, 0x3635, 0x3735, 0x3835, 0x3935, 0x4135, 0x4235,
			0x4335, 0x4435, 0x4535, 0x4635, 0x3036, 0x3136, 0x3236, 0x3336,
			0x3436, 0x3536, 0x3636, 0x3736, 0x3836, 0x3936, 0x4136, 0x4236,
			0x4336, 0x4436, 0x4536, 0x4636, 0x3037, 0x3137, 0x3237, 0x3337,
			0x3437, 0x3537, 0x3637, 0x3737, 0x3837, 0x3937, 0x4137, 0x4237,
			0x4337, 0x4437, 0x4537, 0x4637, 0x3038, 0x3138, 0x3238, 0x3338,
			0x3438, 0x3538, 0x3638, 0x3738, 0x3838, 0x3938, 0x4138, 0x4238,
			0x4338, 0x4438, 0x4538, 0x4638, 0x3039, 0x3139, 0x3239, 0x3339,
			0x3439, 0x3539, 0x3639, 0x3739, 0x3839, 0x3939, 0x4139, 0x4239,
			0x4339, 0x4439, 0x4539, 0x4639, 0x3041, 0x3141, 0x3241, 0x3341,
			0x3441, 0x3541, 0x3641, 0x3741, 0x3841, 0x3941, 0x4141, 0x4241,
			0x4341, 0x4441, 0x4541, 0x4641, 0x3042, 0x3142, 0x3242, 0x3342,
			0x3442, 0x3542, 0x3642, 0x3742, 0x3842, 0x3942, 0x4142, 0x4242,
			0x4342, 0x4442, 0x4542, 0x4642, 0x3043, 0x3143, 0x3243, 0x3343,
			0x3443, 0x3543, 0x3643, 0x3743, 0x3843, 0x3943, 0x4143, 0x4243,
			0x4343, 0x4443, 0x4543, 0x4643, 0x3044, 0x3144, 0x3244, 0x3344,
			0x3444, 0x3544, 0x3644, 0x3744, 0x3844, 0x3944, 0x4144, 0x4244,
			0x4344, 0x4444, 0x4544, 0x4644, 0x3045, 0x3145, 0x3245, 0x3345,
			0x3445, 0x3545, 0x3645, 0x3745, 0x3845, 0x3945, 0x4145, 0x4245,
			0x4345, 0x4445, 0x4545, 0x4645, 0x3046, 0x3146, 0x3246, 0x3346,
			0x3446, 0x3546, 0x3646, 0x3746, 0x3846, 0x3946, 0x4146, 0x4246,
...
inline void convert_bin_to_hex(
		const std::pair<unsigned char*, unsigned char*> &r,
		unsigned char *out) {
...
inline bool convert_hex_to_bin(const unsigned char *begin,
	...
	if (0 == length)
		...
		return false;
	static const unsigned char hex_to_bin[] = { 0x00, 0x00, 0x00, 0x00, 0x00,
	...
	while (end != itr) {
		*reinterpret_cast<unsigned char*>(out) =
				static_cast<unsigned char>(hex_to_bin[itr[0]] << 4
						| hex_to_bin[itr[1]]);
		++out;
		itr += 2;
	}
...
inline bool convert_hex_to_bin(
		const std::pair<unsigned char*, unsigned char*> &r,
		unsigned char *out) {
...
inline bool convert_hex_to_bin(const std::string &hex_data,
	...
	return convert_hex_to_bin(hex_data.data(),
			hex_data.data() + hex_data.size(), const_cast<char*>(output.data()));
}
...
inline std::size_t convert_base64_to_bin(const unsigned char *begin,
		const unsigned char *end, unsigned char *out) {
	static const unsigned char base64_to_bin[] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
			0xFF, 0xFF, 0xFF, // 0x00 - 0x07
...
inline void convert_to_printable_chars(unsigned char *begin,
		unsigned char *end) {
	static const unsigned char printable_char_table[] = { 0x2E, 0x2E, 0x2E,
			0x2E, 0x2E, 0x2E, 0x2E, 0x2E, // 0x00 - 0x07
...
inline bool twoway_bitwise_interleave(const unsigned char *begin1,
		const unsigned char *end1, const unsigned char *begin2,
		const unsigned char *end2, unsigned char *out) {
	if (std::distance(begin1, end1) != std::distance(begin2, end2)) {
		return false;
	}
	...
	while (end1 != itr1) {
		*(reinterpret_cast<unsigned short*>(out)) = (interleave_table[*(itr2++)]
				<< 1);
		*(reinterpret_cast<unsigned short*>(out)) |=
				interleave_table[*(itr1++)];
		out += 2;
	}
	return true;
}
...
inline bool twoway_bitwise_interleave(const char *begin1, const char *end1,
		const char *begin2, const char *end2, char *out) {
	return twoway_bitwise_interleave(
...
inline bool twoway_bitwise_interleave(const std::string &str1,
		const std::string &str2, std::string &out) {
...
template<std::size_t n>
inline void create_nway_interleave_table(
		typename interleave_ary<n>::type table[256]) {
...
inline void bitwise_transform(const bitwise_operation::type &operation,
		...
		const unsigned char *begin2, unsigned char *out) {
	const unsigned char *itr1 = begin1;
	const unsigned char *itr2 = begin2;
...
inline void bitwise_transform(const bitwise_operation::type &operation,
		const char *begin1, const char *end1, const char *begin2, char *out) {
	bitwise_transform(operation, reinterpret_cast<const unsigned char*>(begin1),
			reinterpret_cast<const unsigned char*>(end1),
			reinterpret_cast<const unsigned char*>(begin2),
			reinterpret_cast<unsigned char*>(out));
}
...
inline void bitwise_transform(const bitwise_operation::type &operation,
		const std::string &str1, const std::string &str2, std::string &out) {
	if (str1.size() != str2.size())
		return;
	out.resize(str1.size());
	bitwise_transform(operation, str1.data(), str1.data() + str1.size(),
			str2.data(), const_cast<char*>(out.data()));
}

inline std::size_t high_bit_count(const unsigned char c) {
	static const std::size_t high_bits_in_char[256] = { 0, 1, 1, 2, 1, 2, 2, 3,
...
inline void set_bit_low(const std::size_t &index, unsigned char *const ptr) {
	static const unsigned char bit_mask[] = { 0xFE, //11111110
			0xFD, //11111101
			0xFB, //11111011
			0xF7, //11110111
			0xEF, //11101111
...
inline std::size_t hamming_distance(const unsigned char *begin1,
	...
	while (end1 != itr1) {
		distance += high_bit_count(
				static_cast<unsigned char>(((*itr1++) ^ (*itr2++)) & 0xFF));
	}
...
inline std::size_t hamming_distance(const std::string &str1,
	...
	return hamming_distance(str1.data(), str1.data() + str1.size(), str2.data(),
			str2.data() + str2.size());
}

template<typename Iterator>
inline std::size_t hamming_distance_elementwise(const Iterator begin1,
		const Iterator end1, const Iterator begin2, const Iterator end2) {
	if (std::distance(begin1, end1) != std::distance(begin2, end2)) {
		return std::numeric_limits<std::size_t>::max();
	}
...
private:
	...
	struct store {
		...
		inline bool remove_row_range(const std::size_t &r0,
				const std::size_t &r1) {
			if (r0 > r1)
				return false;
			...
			token_list_t::iterator remove_begin = token_list.begin()
					+ rr0.first;
	...
	struct row_processor {
		row_processor(store &idx, DelimiterPredicate &tp,
				const split_options::type split_mode =
						split_options::compress_delimiters) :
				idx_(idx), row_start_index_(0), row_end_index_(0), token_predicate_(
						tp), split_mode_(split_mode) {
		...
		inline void operator()(const range_t &range) {
			...
			std::size_t token_count = split(token_predicate_, range.first,
					range.second, std::back_inserter(idx_.token_list),
					split_mode_);
			row_end_index_ = row_start_index_ + token_count - 1;
			idx_.row_index.push_back(
					std::make_pair(row_start_index_, row_end_index_));
			if (token_count > idx_.max_column)
				idx_.max_column = token_count;
		}

		row_processor<DelimiterPredicate> operator=(
...
public:
	...
	inline row_range_t range(std::size_t lower_bound, std::size_t upper_bound =
	...
	struct options {
		options() :
				row_split_option(split_options::compress_delimiters), column_split_option(
						split_options::compress_delimiters), row_delimiters(
						"\n\r"), column_delimiters(",|;\t "), support_dquotes(
						false), trim_dquotes(false) {
		}

		options(split_options::type sro, split_options::type sco,
				const std::string &rd, const std::string &cd,
				const bool support_dq = false, const bool trim_dq = false) :
				row_split_option(sro), column_split_option(sco), row_delimiters(
						rd), column_delimiters(cd), support_dquotes(support_dq), trim_dquotes(
						trim_dq) {
		}

		inline options& set_column_split_option(
				const split_options::type &option) {
			column_split_option = option;
			return *this;
		}

		inline options& set_row_split_option(
				const split_options::type &option) {
	...
	public:
		...
		row_type() :
				index_(std::numeric_limits<std::size_t>::max()), size_(0) {
		}

		row_type(const std::size_t &index, const store &dsv_index) :
				index_(index), size_(dsv_index.token_count(index)), begin_(
						dsv_index.token_list.begin()
								+ dsv_index.row_index[index].first) {
		}
		...
		template<typename T0, typename T1, typename T2, typename T3,
				...
				typename T9>
		inline bool parse_with_index(const std::size_t &col0,
				...
				const std::size_t &col9, T0 &t0, T1 &t1, T2 &t2, T3 &t3, T4 &t4,
				T5 &t5, T6 &t6, T7 &t7, T8 &t8, T9 &t9) const {
		...
		inline bool parse_with_index(const std::size_t &col0,
				...
				const std::size_t &col3, const std::size_t &col4,
				const std::size_t &col5, const std::size_t &col6,
				const std::size_t &col7, const std::size_t &col8, T0 &t0,
		...
		inline bool parse_with_index(const std::size_t &col0,
				...
				const std::size_t &col3, const std::size_t &col4,
				const std::size_t &col5, const std::size_t &col6,
				const std::size_t &col7, T0 &t0, T1 &t1, T2 &t2, T3 &t3, T4 &t4,
		...
		inline bool parse_with_index(const std::size_t &col0,
				const std::size_t &col1, const std::size_t &col2,
				const std::size_t &col3, const std::size_t &col4,
				const std::size_t &col5, const std::size_t &col6, T0 &t0,
		...
		inline bool parse_with_index(const std::size_t &col0,
				const std::size_t &col1, const std::size_t &col2,
				const std::size_t &col3, const std::size_t &col4,
				const std::size_t &col5, T0 &t0, T1 &t1, T2 &t2, T3 &t3, T4 &t4,
				T5 &t5) const {
			if (!process(*(begin_ + col0), t0))
				return false;
			if (!process(*(begin_ + col1), t1))
		...
		inline bool parse_with_index(const std::size_t &col0,
				const std::size_t &col1, const std::size_t &col2,
				const std::size_t &col3, const std::size_t &col4, T0 &t0,
		...
		inline bool parse_with_index(const std::size_t &col0,
				const std::size_t &col1, const std::size_t &col2,
				const std::size_t &col3, T0 &t0, T1 &t1, T2 &t2, T3 &t3) const {
			if (!process(*(begin_ + col0), t0))
				return false;
			if (!process(*(begin_ + col1), t1))
				return false;
		...
		inline bool parse_with_index(const std::size_t &col0,
				const std::size_t &col1, const std::size_t &col2, T0 &t0,
				T1 &t1, T2 &t2) const {
			if (!process(*(begin_ + col0), t0))
				return false;
			if (!process(*(begin_ + col1), t1))
		...
		inline bool parse_with_index(const std::size_t &col0,
				const std::size_t &col1, T0 &t0, T1 &t1) const {
			if (!process(*(begin_ + col0), t0))
				return false;
			if (!process(*(begin_ + col1), t1))
				return false;
		...
		template<typename T0, typename T1, typename T2, typename T3,
				typename T4, typename T5, typename T6, typename T7, typename T8,
				typename T9>
		inline bool parse(T0 &t0, T1 &t1, T2 &t2, T3 &t3, T4 &t4, T5 &t5,
		...
		template<typename T0, typename T1, typename T2, typename T3,
				typename T4, typename T5>
		inline bool parse(T0 &t0, T1 &t1, T2 &t2, T3 &t3, T4 &t4,
				T5 &t5) const {
			if (!process(*(begin_ + 0), t0))
				return false;
			if (!process(*(begin_ + 1), t1))
		...
		inline void parse(OutputIterator out) const {
			...
			while (end != itr) {
				const range_t &range = (*itr);
				*(out++) = string_to_type_converter<T>(range.first,
						range.second);
				++itr;
			}
		}
	...
	token_grid() :
			file_name_(""), buffer_(0), buffer_size_(0), min_column_count_(0), max_column_count_(
					0), load_from_file_(false), state_(false) {
	}

	token_grid(const std::string &file_name, const token_grid::options &options) :
			file_name_(file_name), buffer_(0), buffer_size_(0), min_column_count_(
					0), max_column_count_(0), options_(options), load_from_file_(
					true), state_(load()) {
	}
	...
	token_grid(const unsigned char *input_buffer,
			...
			file_name_(""), buffer_(const_cast<unsigned char*>(input_buffer)), buffer_size_(
					input_buffer_size), min_column_count_(0), max_column_count_(
					0), options_(options), load_from_file_(false), state_(
					load()) {
	}

	token_grid(const char *input_buffer, const std::size_t &input_buffer_size,
			const token_grid::options &options) :
			file_name_(""), buffer_(
					reinterpret_cast<unsigned char*>(const_cast<char*>(input_buffer))), buffer_size_(
					input_buffer_size), min_column_count_(0), max_column_count_(
					0), options_(options), load_from_file_(false), state_(
					load()) {
	}
	...
	token_grid(const std::string &input_buffer,
			...
			const token_grid::options &options) :
			file_name_(""), buffer_(
					reinterpret_cast<unsigned char*>(const_cast<char*>(input_buffer.data()))), buffer_size_(
					input_buffer_size), min_column_count_(0), max_column_count_(
					0), options_(options), load_from_file_(false), state_(
					load()) {
	}
	...
	token_grid(const std::string &file_name,
			...
			file_name_(file_name), buffer_(0), buffer_size_(0), min_column_count_(
					0), max_column_count_(0), options_(
					split_options::compress_delimiters,
					split_options::compress_delimiters, row_delimiters,
					column_delimiters), load_from_file_(true), state_(load()) {
	}
	...
	token_grid(const unsigned char *input_buffer,
			...
			file_name_(""), buffer_(const_cast<unsigned char*>(input_buffer)), buffer_size_(
					input_buffer_size), min_column_count_(0), max_column_count_(
					0), options_(split_options::compress_delimiters,
					split_options::compress_delimiters, row_delimiters,
					column_delimiters), load_from_file_(false), state_(load()) {
	}
	...
	inline range_t token(const unsigned int &row,
	...
	inline bool extract_column_checked(const row_range_t &row_range,
			const std::size_t &index, OutputIterator out) const {
	...
	inline bool extract_column(const row_range_t &row_range,
			...
			OutputIterator0 out0, OutputIterator1 out1) const {
		if ((index0 > max_column_count_) || (index1 > max_column_count_))
			return false;
		else if (row_range_invalid(row_range))
			return false;
		...
		for (std::size_t i = row_range.first; i < row_range.second; ++i) {
			...
			if (max_index < dsv_index_.token_count(row)) {
				process_token(
						*(dsv_index_.token_list.begin() + (row.first + index0)),
						out0);
				process_token(
						*(dsv_index_.token_list.begin() + (row.first + index1)),
						out1);
			}
		}
		return true;
	...
	inline bool extract_column(const row_range_t &row_range,
			const std::size_t &index0, const std::size_t &index1,
			const std::size_t &index2, OutputIterator0 out0,
			OutputIterator1 out1, OutputIterator2 out2) const {
		if ((index0 > max_column_count_) || (index1 > max_column_count_)
			...
			if (max_index < dsv_index_.token_count(row)) {
				...
				process_token(
						*(dsv_index_.token_list.begin() + (row.first + index1)),
						out1);
				process_token(
						*(dsv_index_.token_list.begin() + (row.first + index2)),
						out2);
			}
		}
		return true;
	...
	inline bool extract_column(const row_range_t &row_range,
			const std::size_t &index0, const std::size_t &index1,
			const std::size_t &index2, const std::size_t &index3,
			OutputIterator0 out0, OutputIterator1 out1, OutputIterator2 out2,
			OutputIterator3 out3) const {
		if ((index0 > max_column_count_) || (index1 > max_column_count_)
				|| (index2 > max_column_count_) || (index3 > max_column_count_))
			return false;
		else if (row_range_invalid(row_range))
			return false;
		std::size_t max_index = std::max(std::max(index0, index1),
			...
			if (max_index < dsv_index_.token_count(row)) {
				...
				process_token(
						*(dsv_index_.token_list.begin() + (row.first + index2)),
						out2);
				process_token(
						*(dsv_index_.token_list.begin() + (row.first + index3)),
						out3);
			}
		}
		return true;
	...
	inline bool extract_column(const row_range_t &row_range,
			const std::size_t &index0, const std::size_t &index1,
			const std::size_t &index2, const std::size_t &index3,
			const std::size_t &index4, OutputIterator0 out0,
			OutputIterator1 out1, OutputIterator2 out2, OutputIterator3 out3,
		...
		if ((index0 > max_column_count_) || (index1 > max_column_count_)
			...
			return false;
		std::size_t max_index = std::max(index4,
				std::max(std::max(index0, index1), std::max(index2, index3)));
		for (std::size_t i = row_range.first; i < row_range.second; ++i) {
			const row_index_range_t &row = dsv_index_.row_index[i];
			if (max_index < dsv_index_.token_count(row)) {
				process_token(
				...
				process_token(
						*(dsv_index_.token_list.begin() + (row.first + index3)),
						out3);
				process_token(
						*(dsv_index_.token_list.begin() + (row.first + index4)),
						out4);
			}
		}
		return true;
	...
	template<typename Predicate>
	inline bool remove_row_if(const row_range_t &row_range,
			Predicate predicate) {
		...
		for (std::size_t i = row_range.second; i < dsv_index_.row_index.size();
				++i) {
			row_index_range_t &r = dsv_index_.row_index[i];
		...
		if (!remove_row_list.empty()) {
			remove_inplace(index_remover(remove_row_list),
					dsv_index_.row_index);
		}
		if (!remove_token_list.empty()) {
			remove_inplace(index_remover(remove_token_list),
					dsv_index_.token_list);
		}
		return true;
	}
	...
	inline void enforce_min_max_column_count(const row_range_t &row_range,
		...
		if (row_range_invalid(row_range))
			return;
		remove_row_if(
				insufficient_number_of_minmax_columns(min_column_count,
						max_column_count));
		min_column_count_ = min_column_count;
		max_column_count_ = max_column_count;
	}

	inline void enforce_min_max_column_count(
			const std::size_t &min_column_count,
			const std::size_t &max_column_count) {
		enforce_min_max_column_count(all_rows(), min_column_count,
				max_column_count);
	}
	...
	inline std::size_t column_width(const std::size_t &col,
		...
		for (std::size_t i = row_range.first; i < row_range.second; ++i) {
			...
			if (col < dsv_index_.token_count(r)) {
				const range_t &range = *(dsv_index_.token_list.begin() + r.first
						+ col);
				result = std::max<std::size_t>(
						std::distance(range.first, range.second), result);
			}
		}
		return result;
	}
	...
	inline std::size_t accumulate_row(const std::size_t &row, T &result) const {
		...
		token_list_t::const_iterator itr = dsv_index_.token_list.begin()
				+ r.first;
		token_list_t::const_iterator end = dsv_index_.token_list.begin()
				+ r.second + 1;
	...
	inline std::size_t accumulate_column(const std::size_t &col,
			const row_range_t &row_range, T &result) const {
		if (col > max_column_count_)
			return 0;
		else if (row_range_invalid(row_range))
			return 0;
	...
	template<typename T, typename Predicate>
	inline std::size_t accumulate_column(const std::size_t &col,
			const row_range_t &row_range, Predicate p, T &result) const {
		if (col > max_column_count_)
			return 0;
		else if (row_range_invalid(row_range))
			return 0;
	...
	template<typename T, typename Predicate>
	inline std::size_t accumulate_column(const std::size_t &col, Predicate p,
			T &result) const {
		return accumulate_column(col, all_rows(), p, result);
	}

	inline bool join_row(const std::size_t &row, const std::string &delimiter,
			std::string &result) {
		if (row >= dsv_index_.row_index.size())
			return false;
		const row_index_range_t &r = dsv_index_.row_index[row];
		token_list_t::const_iterator itr = dsv_index_.token_list.begin()
				+ r.first;
		token_list_t::const_iterator end = dsv_index_.token_list.begin()
				+ r.second + (row < (dsv_index_.row_index.size() - 1) ? 1 : 0);
		result.reserve(
				delimiter.size() * dsv_index_.token_count(r)
	...
	template<typename Predicate>
	inline bool join_row(const std::size_t &row, Predicate predicate,
	...
	inline bool join_column(const std::size_t &col,
			const row_range_t &row_range, const std::string &delimiter,
	...
	inline bool join_column(const std::size_t &col,
			const std::string &delimiter, std::string &result) const {
		return join_column(col, all_rows(), delimiter, result);
	}

	template<typename Predicate>
	inline bool join_column(const std::size_t &col,
			const row_range_t &row_range, Predicate predicate,
			const std::string &delimiter, std::string &result) const {
		if (col > max_column_count_)
			return false;
		else if (row_range_invalid(row_range))
	...
	template<typename Predicate>
	inline bool join_column(const std::size_t &col, Predicate p,
			const std::string &delimiter, std::string &result) const {
	...
	inline bool sequential_partition(const row_range_t &row_range,
			TransitionPredicate p, Function f) {
		if (row_range_invalid(row_range))
			return false;
		row_range_t r(row_range.first, row_range.first);
		for (row_range_t::first_type i = row_range.first; i < row_range.second;
				++i) {
	...
	bool load(unsigned char *buffer, const std::size_t buffer_size,
			const token_grid::options &options) {
		file_name_ = "";
		if ((load_from_file_) && (0 != buffer_)) {
			delete[] buffer_;
...
private:
	...
	struct insufficient_number_of_minmax_columns {
		insufficient_number_of_minmax_columns(const std::size_t &min_col,
				...
				min_column_count(min_col), max_column_count(max_col) {
		}

		inline bool operator()(const row_type &row) const {
			return (row.size() < min_column_count)
					|| (max_column_count < row.size());
		}
	...
	public:
		...
		double_quotes_predicate(const std::string &delimiters) :
				in_bracket_range_(false), mdp_(delimiters) {
		}
	...
	inline bool load() {
		...
		if (!options_.support_dquotes) {
			multiple_char_delimiter_predicate token_predicate(
					options_.column_delimiters);
			strtk::split(text_newline_predicate, buffer_,
					buffer_ + buffer_size_,
					strtk::functional_inserter(
							row_processor<multiple_char_delimiter_predicate>(
									dsv_index_, token_predicate,
									options_.column_split_option)),
					strtk::split_options::compress_delimiters);
		} else {
			double_quotes_predicate token_predicate_dblq(
					options_.column_delimiters);
			strtk::split(text_newline_predicate, buffer_,
					buffer_ + buffer_size_,
					strtk::functional_inserter(
							row_processor<double_quotes_predicate>(dsv_index_,
									token_predicate_dblq,
									options_.column_split_option)),
					strtk::split_options::compress_delimiters);

			if (options_.trim_dquotes) {
				for (std::size_t i = 0; i < dsv_index_.token_list.size(); ++i) {
					if (((*(dsv_index_.token_list[i].first)) == '"')
	...
	inline void process_token(const range_t &range, OutputIterator out) const {
		...
		(*out) = string_to_type_converter<output_type>(range.first,
				range.second);
		++out;
	}
...
template<typename T>
inline bool convert_string_range(
...
public:
	...
	filter_on_wildcard_match(const std::string &match_pattern,
			OutputPredicate &predicate, bool allow_through_on_match = true) :
			allow_through_on_match_(allow_through_on_match), match_pattern_(
					match_pattern), predicate_(predicate) {
	}
...
public:
	...
	filter_on_match(const std::string *begin, const std::string *end,
			OutputPredicate predicate, bool case_insensitive,
			bool allow_through_on_match = true) :
			case_insensitive_(case_insensitive), allow_through_on_match_(
					allow_through_on_match), begin_(begin), end_(end), predicate_(
					predicate) {
	}
	...
	inline void operator()(const std::pair<Iterator, Iterator> &range) const {
		for (const std::string *itr = begin_; end_ != itr; ++itr) {
			if ((case_insensitive_
					&& (imatch((*itr).data(), (*itr).data() + (*itr).size(),
							range.first, range.second)))
					|| (!case_insensitive_
							&& std::equal((*itr).begin(), (*itr).end(),
									range.first))) {
...
inline void skip_while_matching(Iterator &itr, const Iterator &end,
...
struct size_equal_to {
	...
	inline bool operator()(
			const std::set<T, Comparator, Allocator> &set) const {
		return length == set.size();
	}

	template<typename T, typename Comparator, typename Allocator>
	inline bool operator()(
			const std::multiset<T, Comparator, Allocator> &multiset) const {
...
inline bool parse(const InputIterator begin, const InputIterator end,
		const std::string &delimiters, T1 &t1, T2 &t2, T3 &t3, T4 &t4, T5 &t5,
	...
	const std::size_t parsed_token_count = split_n(delimiters, begin, end,
			token_count, token_list, split_options::compress_delimiters);
...
template<typename InputIterator, typename T1, typename T2, typename T3,
		typename T4>
inline bool parse(const InputIterator begin, const InputIterator end,
		const std::string &delimiters, T1 &t1, T2 &t2, T3 &t3, T4 &t4) {
	typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
	details::convert_type_assert<itr_type>();
...
template<typename InputIterator, typename T1, typename T2, typename T3>
inline bool parse(const InputIterator begin, const InputIterator end,
		const std::string &delimiters, T1 &t1, T2 &t2, T3 &t3) {
	typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
	details::convert_type_assert<itr_type>();
...
inline bool parse(const InputIterator begin, const InputIterator end,
		const std::string &delimiters, T1 &t1, T2 &t2) {
	typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
	details::convert_type_assert<itr_type>();
...
inline bool parse(const InputIterator begin, const InputIterator end,
		const std::string &delimiters, T &t) {
	typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
	details::convert_type_assert<itr_type>();
...
template<typename InputIterator, typename T, typename Allocator, template<
		typename, typename > class Sequence>
inline std::size_t parse(const InputIterator begin, const InputIterator end,
		const std::string &delimiters, Sequence<T, Allocator> &sequence,
	...
	if (1 == delimiters.size())
		return split(
				single_delimiter_predicate<std::string::value_type>(
						delimiters[0]), begin, end,
				range_to_type_back_inserter(sequence), split_option);
	else
		return split(multiple_char_delimiter_predicate(delimiters), begin, end,
				range_to_type_back_inserter(sequence), split_option);
}

template<typename InputIterator, typename T, typename Comparator,
		typename Allocator>
inline std::size_t parse(const InputIterator begin, const InputIterator end,
		const std::string &delimiters, std::set<T, Comparator, Allocator> &set,
		const split_options::type &split_option =
				split_options::compress_delimiters) {
	typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
	details::convert_type_assert<itr_type>();
	if (1 == delimiters.size())
		return split(
				single_delimiter_predicate<std::string::value_type>(
						delimiters[0]), begin, end, range_to_type_inserter(set),
				split_option);
	else
		return split(multiple_char_delimiter_predicate(delimiters), begin, end,
				range_to_type_inserter(set), split_option);
}
...
inline std::size_t parse(const InputIterator begin, const InputIterator end,
	...
	if (1 == delimiters.size())
		return split(
				single_delimiter_predicate<std::string::value_type>(
						delimiters[0]), begin, end,
				range_to_type_inserter(multiset), split_option);
	else
		return split(multiple_char_delimiter_predicate(delimiters), begin, end,
				range_to_type_inserter(multiset), split_option);
}

template<typename InputIterator, typename T, typename Container>
inline std::size_t parse(const InputIterator begin, const InputIterator end,
		const std::string &delimiters, std::queue<T, Container> &queue,
		const split_options::type &split_option =
				split_options::compress_delimiters) {
	typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
	details::convert_type_assert<itr_type>();
	if (1 == delimiters.size())
		return split(
				single_delimiter_predicate<std::string::value_type>(
						delimiters[0]), begin, end,
				range_to_type_push_inserter(queue), split_option);
	else
		return split(multiple_char_delimiter_predicate(delimiters), begin, end,
				range_to_type_push_inserter(queue), split_option);
}
...
template<typename InputIterator, typename T, typename Container>
inline std::size_t parse(const InputIterator begin, const InputIterator end,
		const std::string &delimiters, std::stack<T, Container> &stack,
		const split_options::type &split_option =
				split_options::compress_delimiters) {
	typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
	details::convert_type_assert<itr_type>();
	if (1 == delimiters.size())
		return split(
				single_delimiter_predicate<std::string::value_type>(
						delimiters[0]), begin, end,
				range_to_type_push_inserter(stack), split_option);
	else
		return split(multiple_char_delimiter_predicate(delimiters), begin, end,
				range_to_type_push_inserter(stack), split_option);
}
...
inline std::size_t parse(const InputIterator begin, const InputIterator end,
	...
	if (1 == delimiters.size())
		return split(
				single_delimiter_predicate<std::string::value_type>(
						delimiters[0]), begin, end,
				range_to_type_push_inserter(priority_queue), split_option);
	else
		return split(multiple_char_delimiter_predicate(delimiters), begin, end,
				range_to_type_push_inserter(priority_queue), split_option);
}
...
inline std::size_t parse(const std::pair<InputIterator, InputIterator> &range,
	...
	if (1 == delimiters.size())
		...
		return split(multiple_char_delimiter_predicate(delimiters), range.first,
				range.second, range_to_type_back_inserter(sequence),
				split_option);
}
...
inline std::size_t parse(const std::pair<InputIterator, InputIterator> &range,
	...
	if (1 == delimiters.size())
		...
		return split(multiple_char_delimiter_predicate(delimiters), range.first,
				range.second, range_to_type_inserter(set), split_option);
}
...
inline std::size_t parse(const std::pair<InputIterator, InputIterator> &range,
	...
	if (1 == delimiters.size())
		...
		return split(multiple_char_delimiter_predicate(delimiters), range.first,
				range.second, range_to_type_inserter(multiset), split_option);
}
...
inline std::size_t parse(const std::pair<InputIterator, InputIterator> &range,
	...
	if (1 == delimiters.size())
		...
		return split(multiple_char_delimiter_predicate(delimiters), range.first,
				range.second, range_to_type_push_inserter(queue), split_option);
}
...
inline std::size_t parse(const std::pair<InputIterator, InputIterator> &range,
	...
	if (1 == delimiters.size())
		...
		return split(multiple_char_delimiter_predicate(delimiters), range.first,
				range.second, range_to_type_push_inserter(stack), split_option);
}
...
inline std::size_t parse(const std::pair<InputIterator, InputIterator> &range,
	...
	if (1 == delimiters.size())
		...
		return split(multiple_char_delimiter_predicate(delimiters), range.first,
				range.second, range_to_type_push_inserter(priority_queue),
				split_option);
}
...
private:
	...
	public:
		...
		template<typename InputIterator>
		inline bool add(const InputIterator begin,
	...
	protected:
		...
		virtual bool add_impl(const itr_type begin,
				const itr_type end) const = 0;
	...
	template<typename T, typename Comparator, typename Allocator, template<
			typename, typename, typename > class Set>
...
public:
	...
	container_adder(std::vector<T, Allocator> &vec) :
			container_adder_base_(
					new (buffer) sequence_adder_impl<T, Allocator, std::vector>(
							vec)) {
	}
	...
	container_adder(std::deque<T, Allocator> &deq) :
			container_adder_base_(
					new (buffer) sequence_adder_impl<T, Allocator, std::deque>(
							deq)) {
	}
	...
	container_adder(std::list<T, Allocator> &list) :
			container_adder_base_(
					new (buffer) sequence_adder_impl<T, Allocator, std::list>(
							list)) {
	}
	...
	container_adder(std::set<T, Comparator, Allocator> &set) :
			container_adder_base_(
					new (buffer) set_adder_impl<T, Comparator, Allocator,
							std::set>(set)) {
	}
	...
	container_adder(std::priority_queue<T, Container, Comparator> &pq) :
			container_adder_base_(
					new (buffer) pq_adder_impl<T, Container, Comparator>(pq)) {
	}

	template<typename T, typename Container>
	container_adder(std::queue<T, Container> &queue) :
			container_adder_base_(
					new (buffer) stack_queue_adder_impl<T, Container, std::queue>(
							queue)) {
	}
	...
	container_adder(std::stack<T, Container> &stack) :
			container_adder_base_(
					new (buffer) stack_queue_adder_impl<T, Container, std::stack>(
							stack)) {
	}
...
inline bool parse(const InputIterator begin, const InputIterator end,
	...
	if (1 == delimiters.size())
		...
		split(multiple_char_delimiter_predicate(delimiters), begin, end,
				std::back_inserter(token_list),
				split_options::compress_delimiters);
...
inline bool parse(const InputIterator begin, const InputIterator end,
		const std::string &delimiters, T1 &t1, T2 &t2, T3 &t3, T4 &t4, T5 &t5,
		T6 &t6, T7 &t7, T8 &t8, T9 &t9, T10 &t10, details::container_adder ca) {
	typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
	typedef std::pair<InputIterator, InputIterator> iterator_type;
	typedef typename std::deque<iterator_type>::iterator iterator_type_ptr;
	details::convert_type_assert<itr_type>();
...
inline bool parse(const InputIterator begin, const InputIterator end,
		const std::string &delimiters, T1 &t1, T2 &t2, T3 &t3, T4 &t4, T5 &t5,
		T6 &t6, T7 &t7, T8 &t8, T9 &t9, details::container_adder ca) {
	typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
	typedef std::pair<InputIterator, InputIterator> iterator_type;
	typedef typename std::deque<iterator_type>::iterator iterator_type_ptr;
	details::convert_type_assert<itr_type>();
...
inline bool parse(const InputIterator begin, const InputIterator end,
		const std::string &delimiters, T1 &t1, T2 &t2, T3 &t3, T4 &t4, T5 &t5,
		T6 &t6, T7 &t7, T8 &t8, details::container_adder ca) {
	typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
	typedef std::pair<InputIterator, InputIterator> iterator_type;
	typedef typename std::deque<iterator_type>::iterator iterator_type_ptr;
	details::convert_type_assert<itr_type>();
...
inline bool parse(const InputIterator begin, const InputIterator end,
		const std::string &delimiters, T1 &t1, T2 &t2, T3 &t3, T4 &t4, T5 &t5,
		T6 &t6, T7 &t7, details::container_adder ca) {
	typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
	typedef std::pair<InputIterator, InputIterator> iterator_type;
	typedef typename std::deque<iterator_type>::iterator iterator_type_ptr;
	details::convert_type_assert<itr_type>();
...
inline bool parse(const InputIterator begin, const InputIterator end,
		const std::string &delimiters, T1 &t1, T2 &t2, T3 &t3, T4 &t4, T5 &t5,
		T6 &t6, details::container_adder ca) {
	typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
	typedef std::pair<InputIterator, InputIterator> iterator_type;
	typedef typename std::deque<iterator_type>::iterator iterator_type_ptr;
	details::convert_type_assert<itr_type>();
...
inline bool parse(const InputIterator begin, const InputIterator end,
		const std::string &delimiters, T1 &t1, details::container_adder ca) {
	typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
	typedef std::pair<InputIterator, InputIterator> iterator_type;
	typedef typename std::deque<iterator_type>::iterator iterator_type_ptr;
	details::convert_type_assert<itr_type>();
...
template<typename InputIterator, typename T, typename Allocator, template<
		typename, typename > class Sequence>
inline std::size_t parse_n(const InputIterator begin, const InputIterator end,
		const std::string &delimiters, const std::size_t &n,
		Sequence<T, Allocator> &sequence,
		const split_options::type &split_option =
				split_options::compress_delimiters) {
	typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
	...
	if (1 == delimiters.size())
		split_n(
				single_delimiter_predicate<std::string::value_type>(
						delimiters[0]), begin, end, n,
				range_to_type_back_inserter(sequence), split_option);
	else
		split_n(multiple_char_delimiter_predicate(delimiters), begin, end, n,
				range_to_type_back_inserter(sequence), split_option);
...
template<typename InputIterator, typename T>
inline std::size_t parse_n(const InputIterator begin, const InputIterator end,
		const std::string &delimiters, const std::size_t &n, T *out,
		const split_options::type &split_option =
				split_options::compress_delimiters) {
	typedef typename details::is_valid_iterator<InputIterator>::type itr_type;
	...
	if (1 == delimiters.size())
		split_n(
				single_delimiter_predicate<std::string::value_type>(
						delimiters[0]), begin, end, n,
				range_to_ptr_type(out, insert_count), split_option);
	else
		split_n(multiple_char_delimiter_predicate(delimiters), begin, end, n,
				range_to_ptr_type(out, insert_count), split_option);
	return insert_count;
...
inline bool parse(const std::string &data, const std::string &delimiters,
		T1 &t1, T2 &t2, T3 &t3, T4 &t4, T5 &t5, T6 &t6, T7 &t7, T8 &t8, T9 &t9,
		T10 &t10, T11 &t11, T12 &t12) {
	return parse(data.data(), data.data() + data.size(), delimiters, t1, t2, t3,
			t4, t5, t6, t7, t8, t9, t10, t11,
			typename details::ca_type<T12,
					typename details::is_stl_container<T12>::result_t>::type(
					t12));
}
...
inline bool parse(const std::string &data, const std::string &delimiters,
	...
	return parse(data.data(), data.data() + data.size(), delimiters, t1, t2, t3,
			...
			typename details::ca_type<T11,
					typename details::is_stl_container<T11>::result_t>::type(
					t11));
}
...
inline bool parse(const std::string &data, const std::string &delimiters,
	...
	return parse(data.data(), data.data() + data.size(), delimiters, t1, t2, t3,
			t4, t5, t6, t7, t8, t9,
			typename details::ca_type<T10,
					typename details::is_stl_container<T10>::result_t>::type(
					t10));
}
...
inline bool parse(const std::string &data, const std::string &delimiters,
	...
	return parse(data.data(), data.data() + data.size(), delimiters, t1, t2, t3,
			t4, t5, t6, t7, t8,
			typename details::ca_type<T9,
					typename details::is_stl_container<T9>::result_t>::type(t9));
}
...
inline bool parse(const std::string &data, const std::string &delimiters,
	...
	return parse(data.data(), data.data() + data.size(), delimiters, t1, t2, t3,
			t4, t5, t6, t7,
			typename details::ca_type<T8,
					typename details::is_stl_container<T8>::result_t>::type(t8));
}
...
inline bool parse(const std::string &data, const std::string &delimiters,
	...
	return parse(data.data(), data.data() + data.size(), delimiters, t1, t2, t3,
			t4, t5, t6,
			typename details::ca_type<T7,
					typename details::is_stl_container<T7>::result_t>::type(t7));
}
...
inline bool parse(const std::string &data, const std::string &delimiters,
	...
	return parse(data.data(), data.data() + data.size(), delimiters, t1, t2, t3,
			t4, t5,
			typename details::ca_type<T6,
					typename details::is_stl_container<T6>::result_t>::type(t6));
}
...
inline bool parse(const std::string &data, const std::string &delimiters,
	...
	return parse(data.data(), data.data() + data.size(), delimiters, t1, t2, t3,
			t4,
			typename details::ca_type<T5,
					typename details::is_stl_container<T5>::result_t>::type(t5));
}
...
inline bool parse(const std::string &data, const std::string &delimiters,
	...
	return parse(data.data(), data.data() + data.size(), delimiters, t1, t2, t3,
			typename details::ca_type<T4,
					typename details::is_stl_container<T4>::result_t>::type(t4));
}

template<typename T1, typename T2, typename T3>
inline bool parse(const std::string &data, const std::string &delimiters,
		T1 &t1, T2 &t2, T3 &t3) {
	return parse(data.data(), data.data() + data.size(), delimiters, t1, t2,
			typename details::ca_type<T3,
					typename details::is_stl_container<T3>::result_t>::type(t3));
}

template<typename T1, typename T2>
inline bool parse(const std::string &data, const std::string &delimiters,
		T1 &t1, T2 &t2) {
	return parse(data.data(), data.data() + data.size(), delimiters, t1,
			typename details::ca_type<T2,
					typename details::is_stl_container<T2>::result_t>::type(t2));
}
...
inline bool parse(const std::string &data, const std::string &delimiters,
	...
	return parse(data.data(), data.data() + data.size(), delimiters,
			typename details::ca_type<T,
					typename details::is_stl_container<T>::result_t>::type(t));
}

template<typename T, typename Allocator,
		template<typename, typename > class Sequence>
inline std::size_t parse(const std::string &data, const std::string &delimiters,
		Sequence<T, Allocator> &sequence,
		const split_options::type &split_option =
				split_options::compress_delimiters) {
	return parse(data.data(), data.data() + data.size(), delimiters, sequence,
			split_option);
}
...
inline std::size_t parse(const std::string &data, const std::string &delimiters,
		...
		const split_options::type &split_option =
				split_options::compress_delimiters) {
	return parse(data.data(), data.data() + data.size(), delimiters, set,
			split_option);
}
...
inline std::size_t parse(const std::string &data, const std::string &delimiters,
		...
		const split_options::type &split_option =
				split_options::compress_delimiters) {
	return parse(data.data(), data.data() + data.size(), delimiters, multiset,
			split_option);
}
...
inline std::size_t parse(const std::string &data, const std::string &delimiters,
		...
		const split_options::type &split_option =
				split_options::compress_delimiters) {
	return parse(data.data(), data.data() + data.size(), delimiters, queue,
			split_option);
}
...
inline std::size_t parse(const std::string &data, const std::string &delimiters,
		...
		const split_options::type &split_option =
				split_options::compress_delimiters) {
	return parse(data.data(), data.data() + data.size(), delimiters, stack,
			split_option);
}
...
inline std::size_t parse(const std::string &data, const std::string &delimiters,
		...
		const split_options::type &split_option =
				split_options::compress_delimiters) {
	return parse(data.data(), data.data() + data.size(), delimiters,
			priority_queue, split_option);
}

template<typename T, typename Allocator,
		template<typename, typename > class Sequence>
inline std::size_t parse(const int &argc, char *argv[],
		Sequence<T, Allocator> &sequence, const bool break_on_fail = true) {
	T tmp;
...
template<typename T1, typename T2, typename T3, typename T4, typename T5,
		typename T6, typename T7, typename T8, typename T9>
inline std::size_t parse(const int &argc, char *argv[], T1 &t1, T2 &t2, T3 &t3,
...
template<typename T1, typename T2, typename T3>
inline std::size_t parse(const int &argc, char *argv[], T1 &t1, T2 &t2,
		T3 &t3) {
	if (3 != argc)
		return 0;
	std::size_t result = 0;
...
inline std::size_t parse(const int &argc, char *argv[], T1 &t1) {
	if (1 != argc)
		return 0;
	std::size_t result = 0;
	if (!string_to_type_converter(std::string(argv[0]), t1))
...
inline std::size_t parse_n(const std::string &data,
		...
		const split_options::type &split_option =
				split_options::compress_delimiters) {
	return parse_n(data.data(), data.data() + data.size(), delimiters, n,
			sequence, split_option);
}
...
inline std::size_t parse_n(const std::string &data,
		...
		const split_options::type &split_option =
				split_options::compress_delimiters) {
	return parse_n(data.data(), data.data() + data.size(), delimiters, n, set,
			split_option);
}
...
inline std::size_t parse_n(const std::string &data,
		...
		const split_options::type &split_option =
				split_options::compress_delimiters) {
	return parse_n(data.data(), data.data() + data.size(), delimiters, n, queue,
			split_option);
}
...
inline std::size_t parse_n(const std::string &data,
		...
		const split_options::type &split_option =
				split_options::compress_delimiters) {
	return parse_n(data.data(), data.data() + data.size(), delimiters, n, stack,
			split_option);
}
...
inline std::size_t parse_n(const std::string &data,
		...
		const split_options::type &split_option =
				split_options::compress_delimiters) {
	return parse_n(data.data(), data.data() + data.size(), delimiters, n, out,
			split_option);
}
...
template<typename T1, typename T2, typename T3, typename T4, typename T5,
		typename T6, typename T7, typename T8, typename T9, typename T10,
		typename T11, typename T12>
inline bool parse_line(std::ifstream &stream, const std::string &delimiters,
		T1 &t1, T2 &t2, T3 &t3, T4 &t4, T5 &t5, T6 &t6, T7 &t7, T8 &t8, T9 &t9,
		T10 &t10, T11 &t11, T12 &t12) {
	if (!stream)
		return false;
	...
	if (data.empty() || delimiters.empty())
		return false;
	return strtk::parse(data.data(), data.data() + data.size(), delimiters, t1,
...
inline std::size_t parse_line(std::ifstream &stream,
	...
	if (data.empty() || delimiters.empty())
		return false;
	return strtk::parse(data.data(), data.data() + data.size(), delimiters,
			sequence, split_option);
}
...
inline std::size_t parse_line(std::ifstream &stream,
	...
	if (data.empty() || delimiters.empty())
		return false;
	return strtk::parse(data.data(), data.data() + data.size(), delimiters, set,
			split_option);
}
...
inline std::size_t parse_line_n(std::ifstream &stream,
	...
	if (data.empty() || delimiters.empty())
		return 0;
	return strtk::parse_n(data.data(), data.data() + data.size(), delimiters, n,
			sequence, split_option);
}
...
inline void join(std::string &output, const std::string &delimiter, int argc,
		char *argv[]) {
	for (int i = 0; i < argc; ++i) {
		output += argv[i];
		if (i < (argc - 1))
...
template<typename InputIterator>
inline std::string join(const std::string &delimiter, const InputIterator begin,
		const InputIterator end) {
	std::string output;
...
inline void join_if(std::string &output, const std::string &delimiter,
		Predicate predicate, const InputIterator begin,
		const InputIterator end) {
	InputIterator itr = begin;
...
template<typename T, typename Predicate, typename Allocator, template<typename,
		typename > class Sequence>
inline void join_if(std::string &output, const std::string &delimiter,
		Predicate predicate, const Sequence<T, Allocator> &sequence) {
	join_if(output, delimiter, predicate, sequence.begin(), sequence.end());
}

template<typename T, typename Predicate, typename Comparator, typename Allocator>
inline void join_if(std::string &output, const std::string &delimiter,
		Predicate predicate, const std::set<T, Comparator, Allocator> &set) {
...
template<typename InputIterator, typename Predicate>
inline std::string join_if(const std::string &delimiter, Predicate predicate,
		const InputIterator begin, const InputIterator end) {
	std::string output;
	output.reserve(one_kilobyte);
...
inline void replicate(const std::size_t &n, const std::string &str,
		std::string &output) {
	if (0 == n)
		return;
...
inline std::string replicate(const std::size_t &n, const std::string &str) {
	std::string output;
	replicate(n, str, output);
	return output;
}

inline void replicate_inplace(const std::size_t &n, std::string &str) {
...
template<typename InputIterator>
inline void bracketize(std::string &output, const std::string &pre,
		const std::string &post, const InputIterator begin,
		const InputIterator end) {
	InputIterator itr = begin;
	std::string s;
...
template<typename T, typename Allocator,
		template<typename, typename > class Sequence>
inline void bracketize(std::string &output, const std::string &pre,
		const std::string &post, Sequence<T, Allocator> &sequence) {
	bracketize(output, pre, post, sequence.begin(), sequence.end());
}

template<typename T, typename Comparator, typename Allocator>
inline void bracketize(std::string &output, const std::string &pre,
		const std::string &post, std::set<T, Comparator, Allocator> &set) {
...
template<typename InputIterator>
inline std::string bracketize(const std::string &pre, const std::string &post,
		const InputIterator begin, const InputIterator end) {
	std::string output;
	output.reserve(one_kilobyte);
...
struct interval_inserter {
	...
	interval_inserter(const std::size_t &interval, const T &t) :
			count_(0), interval_(interval), t_(t) {
	}
...
template<typename Inserter, typename InputIterator, typename OutputIterator>
inline std::size_t inserter(Inserter ins, const InputIterator begin,
		const InputIterator end, OutputIterator out) {
	std::size_t size = 0;
	InputIterator itr = begin;
	while (end != itr) {
...
inline void cut(const std::size_t &r0, const std::size_t &r1,
		const InputIterator begin, InputIterator end, OutputIterator out) {
...
template<typename Allocator, template<typename, typename > class Sequence,
		typename OutputIterator>
inline void cut(const std::size_t &r0, const std::size_t &r1,
		const Sequence<std::string, Allocator> &sequence, OutputIterator out) {
...
public:
	...
	translation_table(const std::string &itable, const std::string &otable) {
		...
		for (std::size_t i = 0; i < itable.size(); ++i) {
			table_[static_cast<unsigned int>(itable[i])] =
					static_cast<unsigned char>(otable[i]);
		}
	}
...
#ifdef strtk_enable_random
inline void generate_random_data(unsigned char *data, std::size_t length,
		unsigned int pre_gen_cnt = 0, unsigned int seed = magic_seed) {
	boost::mt19937 rng(static_cast<boost::mt19937::result_type>(seed));
	boost::uniform_int<unsigned int> dist(
			std::numeric_limits<unsigned int>::min(),
			std::numeric_limits<unsigned int>::max());
	boost::variate_generator<boost::mt19937&, boost::uniform_int<unsigned int> > rnd(
			rng, dist);
...
inline void generate_random_values_impl(const std::size_t &count, const T &min,
		const T &max, OutputIterator out, RandomNumberGenerator &rng,
		rand_int_type_tag) {
...
public:
	...
	uniform_real_rng(const std::size_t &seed = magic_seed, std::size_t pregen =
			0) :
			rng_(static_cast<rng_type::result_type>(seed)), rnd_(rng_,
...
template<typename T, typename OutputIterator>
inline void generate_random_values(const std::size_t &count, const T &min,
		const T &max, OutputIterator out, const std::size_t &seed = magic_seed,
...
template<typename T, typename Allocator,
		template<typename, typename > class Sequence>
inline void generate_random_values(const std::size_t &count, const T &min,
		const T &max, Sequence<T, Allocator> &sequence,
		const std::size_t &seed = magic_seed, const std::size_t &pregen = 0) {
	typename details::supported_random_type<T>::type type;
	boost::mt19937 rng(static_cast<boost::mt19937::result_type>(seed));
	for (std::size_t i = 0; i++ < pregen; rng())
		;
...
inline void random_permutation(const Iterator begin, const Iterator end,
		RandomNumberGenerator &rng, OutputIterator out) {
...
template<typename Iterator, typename OutputIterator>
inline void random_permutation(const Iterator begin, const Iterator end,
		OutputIterator out, const std::size_t &seed = magic_seed,
		const std::size_t &pregen = 0) {
	boost::mt19937 rng(static_cast<boost::mt19937::result_type>(seed));
	for (std::size_t i = 0; i++ < pregen; rng())
		;
...
template<typename T, typename Allocator,
		template<typename, typename > class Sequence, typename OutputIterator>
...
inline bool random_combination(const Iterator begin, const Iterator end,
		std::size_t set_size, RandomNumberGenerator &rng, OutputIterator out) {
	const std::size_t size = std::distance(begin, end);
...
inline void random_combination(const Iterator begin, const Iterator end,
		const std::size_t &set_size, OutputIterator out,
		const std::size_t &seed = magic_seed, const std::size_t &pregen = 0) {
	boost::mt19937 rng(static_cast<boost::mt19937::result_type>(seed));
	for (std::size_t i = 0; i++ < pregen; rng())
		;
...
inline std::size_t select_k_randomly(const Iterator begin, const Iterator end,
		const std::size_t k, OutputIterator out, RandomNumberGenerator &rng) {
...
inline void select_1_randomly(const Iterator begin, const Iterator end,
		OutputIterator out, RandomNumberGenerator &rng) {
	typedef typename std::iterator_traits<Iterator>::value_type T;
	T selection;
	if (begin == end)
		return;
...
inline bool combine_discontinuous_conditional(Iterator first1, Iterator last1,
	...
	if (d1 == 1) {
		...
		for (D d22 = d2; i2 != last2; ++i2, --d22) {
			if (!combine_discontinuous_conditional(f1p, last1, d1 - 1, i2,
					last2, d22, f, d + 1))
				return false;
			swap(*first1, *i2);
		}
...
inline void for_each_combination(Iterator begin, Iterator end,
		const std::size_t &size, Function function) {
	if (static_cast<typename std::iterator_traits<Iterator>::difference_type>(size)
			> std::distance(begin, end))
		return;
	Iterator mid = begin + size;
	details::bound_range<Function&, Iterator> func(function, begin, mid);
	details::combine_discontinuous(begin, mid, std::distance(begin, mid), mid,
			end, std::distance(mid, end), func);
}
...
inline void for_each_combination_conditional(Iterator begin, Iterator end,
	...
	details::combine_discontinuous_conditional(begin, mid,
			std::distance(begin, mid), mid, end, std::distance(mid, end), func);
}
...
inline unsigned long long int n_choose_k(const unsigned long long int &n,
	...
	public:
		...
		n_choose_k_impl(value_type *table, const value_type &dimension) :
				table_(table), dimension_(dimension / 2) {
		}
	...
	static const std::size_t static_table_size =
			static_cast<std::size_t>((static_table_dim * static_table_dim) / 2);
...
inline void nth_combination_sequence(unsigned long long int n,
		const std::size_t &r, const std::size_t &k, OutputIterator out,
	...
	index_list[k - 1] = index_list[k - 2] + static_cast<std::size_t>(n)
...
template<typename InputIterator, typename OutputIterator>
inline void nth_combination_sequence(const std::size_t &n, const std::size_t &k,
		const InputIterator begin, const InputIterator end, OutputIterator out,
		const bool complete_index = true) {
	const std::size_t length = std::distance(begin, end);
...
template<typename InputIterator, typename OutputIterator>
inline void nth_permutation_sequence(std::size_t n, const InputIterator begin,
		const InputIterator end, OutputIterator out) {
	const std::size_t size = std::distance(begin, end);
...
class combination_iterator: public std::iterator<std::forward_iterator_tag,
		std::pair<Iterator, Iterator>, void, void> {
public:
	...
	explicit inline combination_iterator(const std::size_t &k, iterator begin,
			iterator end, const bool sorted = true) :
			begin_(begin), end_(end), middle_(begin + k), current_combination_(
					begin_, middle_) {
		if (!sorted) {
			std::sort(begin, end);
		}
	...
	explicit inline combination_iterator(const std::size_t &k,
			Sequence<T, Allocator> &seq, const bool sorted = true) :
			begin_(seq.begin()), end_(seq.end()), middle_(begin_ + k), current_combination_(
					begin_, middle_) {
		if (!sorted) {
	...
	explicit inline combination_iterator(const std::size_t &k, std::string &str,
			const bool sorted = true) :
			begin_(const_cast<char*>(str.data())), end_(
					const_cast<char*>(str.data() + str.size())), middle_(
					begin_ + k), current_combination_(begin_, middle_) {
		if (!sorted) {
			std::sort(begin_, end_);
		}
	}
	...
	inline combination_iterator(iterator end) :
			begin_(end), end_(end), middle_(end), current_combination_(end, end) {
	}

	inline combination_iterator(const std::string &str) :
			begin_(const_cast<char*>(str.data() + str.size())), end_(begin_), middle_(
					end_), current_combination_(end_, end_) {
	}

	template<typename T, typename Allocator,
			template<typename, typename > class Sequence>
	explicit inline combination_iterator(Sequence<T, Allocator> &seq) :
			begin_(seq.end()), end_(seq.end()), middle_(end_), current_combination_(
					end_, end_) {
	}
	...
	inline bool operator==(const combination_iterator &itr) const {
		return (begin_ == itr.begin_) && (end_ == itr.end_)
				&& (middle_ == itr.middle_);
	}
...
struct numeric_convert_impl<T, Iterator, 19> {
	static inline void process(Iterator itr, T &t) {
		strtk::fast::details::numeric_convert_impl<T, Iterator, 18>::process(
				itr + 1, t);
...
struct numeric_convert_impl<T, Iterator, 18> {
	static inline void process(Iterator itr, T &t) {
		strtk::fast::details::numeric_convert_impl<T, Iterator, 17>::process(
				itr + 1, t);
...
inline void numeric_convert(const std::string &s, T &t, const bool digit_check =
		false) {
	numeric_convert<N, T, const char*>(s.data(), t, digit_check);
}

template<typename T, typename Iterator>
inline bool numeric_convert(const std::size_t &n, Iterator itr, T &t,
		const bool digit_check = false) {
	if (digit_check) {
...
template<std::size_t N, typename T, typename Iterator>
inline void signed_numeric_convert(Iterator itr, T &t, const bool digit_check =
		false) {
	if ('-' == (*itr)) {
...
template<typename T, typename Iterator>
inline bool signed_numeric_convert(const std::size_t &n, Iterator itr, T &t,
		const bool digit_check = false) {
	if ('-' == (*itr)) {
...
template<std::size_t N, typename T>
inline void signed_numeric_convert(const std::string &s, T &t,
		const bool digit_check = false) {
	signed_numeric_convert<N, T, const char*>(s.data(), t, digit_check);
}

template<typename T>
inline bool signed_numeric_convert(const std::string &s, T &t,
		const bool digit_check = false) {
...
static inline unsigned long long int convert(const unsigned long long int v) {
	...
	return ((v >> 56) & 0x00000000000000FFLL)
			| ((v << 56) & 0xFF00000000000000LL)
			| ((v >> 40) & 0x000000000000FF00LL)
			| ((v << 40) & 0x00FF000000000000LL)
			| ((v >> 24) & 0x0000000000FF0000LL)
			| ((v << 24) & 0x0000FF0000000000LL)
			| ((v >> 8) & 0x00000000FF000000LL)
...
static inline long long int convert(const long long int v) {
	return static_cast<long long>(convert(
...
static inline unsigned long long int convert_to_be(
...
static inline unsigned long long int convert_to_le(
		const unsigned long long int v) {
	return (is_big_endian()) ? convert(v) : v;
}
...
public:
	...
	reader(T *buffer, const std::size_t &buffer_length) :
			original_buffer_(reinterpret_cast<char*>(buffer)), buffer_(
					reinterpret_cast<char*>(buffer)), buffer_length_(
					buffer_length * sizeof(T)), amount_read_sofar_(0) {
	}

	inline bool operator!() const {
		return (0 == buffer_length_) || (0 == original_buffer_)
				|| (0 == buffer_);
	}
	...
	template<typename T>
	inline bool operator()(T *&data, uint32_t &length, const bool read_length =
			true) {
	...
	template<typename T>
	inline bool operator()(T *&data, uint64_t &length, const bool read_length =
			true) {
		uint32_t l = 0;
		if (read_length)
	...
	inline bool operator()(std::string &output) {
		...
		std::copy(buffer_, buffer_ + length, const_cast<char*>(output.data()));
		buffer_ += length;
	...
	inline bool operator()(T (&output)[N]) {
		...
		if (buffer_capacity_ok(raw_size)) {
			std::copy(buffer_, buffer_ + raw_size,
...
private:
	...
	private:
		...
		struct selector_impl {
			...
			template<typename Reader, typename Allocator>
			static inline bool batch_vector_read(Reader &r,
					const std::size_t &size, std::vector<T, Allocator> &v,
					const bool) {
				T t;
				for (std::size_t i = 0; i < size; ++i) {
					if (r.operator()(t))
		...
		struct selector_impl<T, strtk::details::yes_t> {
			...
			static inline bool batch_vector_read(Reader &r,
				...
				char *ptr =
						const_cast<char*>(reinterpret_cast<const char*>(&v[0]));
	...
	public:
		...
		typedef selector_impl<Type,
				typename strtk::details::is_pod<Type>::result_t> type;
	};

	template<typename T>
	inline bool read_pod(T &data, const bool perform_buffer_capacity_check =
			true) {
...
public:
	...
	writer(T *buffer, const std::size_t &buffer_length) :
			original_buffer_(reinterpret_cast<char*>(buffer)), buffer_(
					reinterpret_cast<char*>(buffer)), buffer_length_(
					buffer_length * sizeof(T)), amount_written_sofar_(0) {
	}
	...
	template<typename T, std::size_t N>
	inline bool operator()(const T (&data)[N],
			const bool write_length = false) {
	...
	inline bool operator()(const std::set<T, Comparator, Allocator> &set) {
		...
		typename std::set<T, Comparator, Allocator>::const_iterator itr =
				set.begin();
		typename std::set<T, Comparator, Allocator>::const_iterator end =
				set.end();
		while (end != itr) {
			if (!operator()(*itr))
				return false;
	...
	template<typename T>
	inline bool operator()(const T &input, const std::size_t &size,
			const padding_mode pmode, const char padding = ' ') {
		if (amount_written_sofar_ + size <= buffer_length_) {
			...
			return operator()<const char>(s.data(), static_cast<uint32_t>(size),
					false);
		} else
			return false;
	}
	...
	private:
		...
		struct selector_impl {
			...
			static inline bool batch_vector_writer(Writer &w,
					const std::size_t&, const std::vector<T, Allocator> &v) {
...
public:
	...
	template<typename InputIterator>
	inline ignore_token& operator=(
			const std::pair<InputIterator, InputIterator>&) {
		return (*this);
	}
...
public:
	...
	template<typename InputIterator>
	inline hex_to_number_sink& operator=(
			const std::pair<InputIterator, InputIterator> &s) {
		std::size_t offset = 0;
...
public:
	...
	inline base64_to_number_sink& operator=(const std::string &s) {
		...
		(*t_) = T(0);
		convert_base64_to_bin(s.data(), s.data() + s.size(),
				reinterpret_cast<char*>(t_));
		reverse_bytes();
		return (*this);
	}
	...
	template<typename InputIterator>
	inline base64_to_number_sink& operator=(
			const std::pair<InputIterator, InputIterator> &s) {
...
public:
	...
	template<typename InputIterator>
	inline hex_to_string_sink& operator=(
			const std::pair<InputIterator, InputIterator> &s) {
		const std::size_t size = std::distance(s.first, s.second);
		std::size_t offset = 0;
		if ((size > 2) && ((*s.first) == '0')
		...
		valid_ = convert_hex_to_bin(s.first + offset, s.second,
	...
	inline hex_to_string_sink& operator=(const std::string &s) {
		return this->operator=(
				std::make_pair<char*>(const_cast<char*>(s.data()),
						const_cast<char*>(s.data() + s.size())));
	}
...
public:
	...
	truncated_int() :
			t_(0), fractional_size_(std::numeric_limits<std::size_t>::max()) {
	}
...
inline std::size_t parse_stl_container_proxy(const InputIterator begin,
		const InputIterator end, const std::string &delimiters,
		Sequence<T, Allocator> &sequence,
		const split_options::type &split_option =
				split_options::compress_delimiters) {
...
inline std::size_t parse_n_stl_container_proxy(const InputIterator begin,
		const InputIterator end, const std::string &delimiters,
		const std::size_t &n, std::set<T, Comparator, Allocator> &set,
		const split_options::type &split_option =
				split_options::compress_delimiters) {
...
inline std::size_t parse_n_stl_container_proxy(const InputIterator begin,
		const InputIterator end, const std::string &delimiters,
		const std::size_t &n, std::multiset<T, Comparator, Allocator> &multiset,
		const split_options::type &split_option =
				split_options::compress_delimiters) {
...
inline std::size_t parse_n_stl_container_proxy(const InputIterator begin,
		const InputIterator end, const std::string &delimiters,
		const std::size_t &n, std::queue<T, Container> &queue,
		const split_options::type &split_option =
				split_options::compress_delimiters) {
...
inline std::size_t parse_n_stl_container_proxy(const InputIterator begin,
		const InputIterator end, const std::string &delimiters,
		const std::size_t &n, std::stack<T, Container> &stack,
		const split_options::type &split_option =
				split_options::compress_delimiters) {
...
public:
	...
	inline sink_type(const std::string &delimiters,
			const split_options::type &split_option =
					split_options::compress_delimiters) :
			delimiters_(delimiters), split_option_(split_option), container_(0), element_count_(
					std::numeric_limits<std::size_t>::max()) {
	}

	inline sink_type(Container &container, const std::string &delimiters,
			const split_options::type &split_option =
					split_options::compress_delimiters) :
			delimiters_(delimiters), split_option_(split_option), container_(
					&container) {
	}
	...
	inline bool parse(InputIterator begin, InputIterator end) {
		if (container_) {
			if (std::numeric_limits<std::size_t>::max() == element_count_)
				return (details::parse_stl_container_proxy(begin, end,
						delimiters_, (*container_), split_option_) > 0);
			else
				return (details::parse_n_stl_container_proxy(begin, end,
						delimiters_, element_count_, (*container_),
...
inline std::string center(const std::size_t &width,
		const std::string::value_type &pad, const std::string &str) {
	if (str.size() >= width)
		return str;
	const std::size_t pad_count = width - str.size();
	const std::size_t pad_count_2 = (pad_count >> 1) + (pad_count & 1);
	return std::string(pad_count >> 1, pad) + str
			+ std::string(pad_count_2, pad);
}
...
template<typename T>
inline std::string center(const std::size_t &width,
		const std::string::value_type &pad, const T &t) {
...
inline void remaining_string(const std::size_t &index, const std::string &str,
		std::string &result) {
...
namespace find_type {
enum type {
	digits, letters, lowercase_letters, uppercase_letters, letters_digits
};
}
...
inline typename range_type<Iterator>::type find_exactly_n_consecutive_values(
		const std::size_t n, Predicate p, Iterator itr, const Iterator end,
...
template<typename Iterator, typename Predicate>
inline typename range_type<Iterator>::type find_exactly_n_consecutive_values(
		const std::size_t n, Predicate p,
...
inline typename range_type<Iterator>::type find_n_consecutive_values(
		const std::size_t n, find_mode::type mode, Predicate p, Iterator itr,
		const Iterator end) {
	switch (mode) {
...
template<typename Iterator, typename Predicate>
inline bool match_exactly_n_consecutive_values(const std::size_t n, Predicate p,
...
template<typename Iterator, typename Predicate>
inline bool match_atleast_n_consecutive_values(const std::size_t n, Predicate p,
		Iterator itr, const Iterator end) {
	if (static_cast<unsigned int>(std::distance(itr, end)) < n)
		return false;
	std::size_t count = 0;
...
inline bool match_n_consecutive_values(const std::size_t n,
		find_mode::type mode, Predicate p, Iterator itr, const Iterator end) {
	switch (mode) {
	case find_mode::exactly_n:
...
inline typename details::range_type<Iterator>::type find_n_consecutive(
		const std::size_t n, find_type::type type, find_mode::type mode,
		typename details::range_type<Iterator>::type range) {
	switch (type) {
	case find_type::digits:
		return details::find_n_consecutive_values<Iterator>(n, mode,
				strtk::text::is_digit, range.first, range.second);

	case find_type::letters:
		return details::find_n_consecutive_values<Iterator>(n, mode,
				strtk::text::is_letter, range.first, range.second);

	case find_type::lowercase_letters:
		return details::find_n_consecutive_values<Iterator>(n, mode,
				strtk::text::is_lowercase_letter, range.first, range.second);

	case find_type::uppercase_letters:
		return details::find_n_consecutive_values<Iterator>(n, mode,
				strtk::text::is_uppercase_letter, range.first, range.second);

	case find_type::letters_digits:
		return details::find_n_consecutive_values<Iterator>(n, mode,
				strtk::text::is_letter_or_digit, range.first, range.second);
...
template<typename Iterator>
inline bool match_n_consecutive(const std::size_t n, find_type::type type,
		find_mode::type mode,
		typename details::range_type<Iterator>::type range) {
	switch (type) {
	case find_type::digits:
		return details::match_n_consecutive_values<Iterator>(n, mode,
				strtk::text::is_digit, range.first, range.second);
...
inline std::size_t split_on_consecutive(const std::size_t n, Predicate p,
		char *begin, char *end, OutputIterator out,
		const bool stateful_predicate = false) {
	if (0 == n)
		return 0;
	...
	while (end != itr_range.first) {
		range_type found_itr = details::find_exactly_n_consecutive_values<
				iterator_type, Predicate>(n, p, itr_range, stateful_predicate);
...
inline std::size_t split_on_consecutive(const std::size_t n,
		const std::size_t m, Predicate p, char *begin, char *end,
		OutputIterator out) {
	if (0 == n)
		return 0;
	typedef char *iterator_type;
...
template<typename InputIterator, typename OutputIterator>
inline std::size_t split_on_consecutive(const std::size_t &n,
		const find_type::type type, const find_mode::type mode, char *begin,
		char *end, OutputIterator out) {
	if (0 == n)
		return 0;
	typedef char *iterator_type;
	typedef details::range_type<iterator_type>::type range_type;
...
inline std::size_t split_on_consecutive_n(const std::size_t &n,
		const std::size_t &m, const find_type::type type,
		const find_mode::type mode, char *begin, char *end,
		OutputIterator out) {
	if (0 == n)
		return 0;
	typedef char *iterator_type;
...
template<typename OutputIterator>
inline std::size_t split_on_consecutive(const std::size_t &n,
		const find_type::type type, const find_mode::type mode,
		const char *begin, const char *end, OutputIterator out) {
	return split_on_consecutive<char*, OutputIterator>(n, type, mode,
			const_cast<char*>(begin), const_cast<char*>(end), out);
}
...
inline std::size_t split_on_consecutive(const std::size_t &n,
		...
		OutputIterator out) {
	return split_on_consecutive<OutputIterator>(n, type, mode,
			reinterpret_cast<const char*>(begin),
			reinterpret_cast<const char*>(end), out);
}
...
inline std::size_t split_on_consecutive(const std::size_t &n,
		const find_type::type type, const find_mode::type mode,
		const std::string &str, OutputIterator out) {
	return split_on_consecutive<OutputIterator>(n, type, mode, str.data(),
...
inline std::size_t split_on_consecutive_n(const std::size_t &n,
		const std::size_t &m, const find_type::type type,
		const find_mode::type mode, const char *begin, const char *end,
		OutputIterator out) {
	return split_on_consecutive_n<char*, OutputIterator>(n, m, type, mode,
			const_cast<char*>(begin), const_cast<char*>(end), out);
}
...
inline std::size_t split_on_consecutive_n(const std::size_t &n,
		const std::size_t &m, const find_type::type type,
		const find_mode::type mode, const unsigned char *begin,
		const unsigned char *end, OutputIterator out) {
	return split_on_consecutive_n<OutputIterator>(n, m, type, mode,
			reinterpret_cast<const char*>(begin),
			reinterpret_cast<const char*>(end), out);
}
...
inline std::size_t split_on_consecutive_n(const std::size_t &n,
		const std::size_t &m, const find_type::type type,
		const find_mode::type mode, const std::string &str,
		OutputIterator out) {
	return split_on_consecutive_n<OutputIterator>(n, m, type, mode, str.data(),
			str.data() + str.size(), out);
}
...
inline std::size_t split_on_consecutive(const std::size_t &n, Predicate p,
		const char *begin, const char *end, OutputIterator out,
		const bool stateful_predicate = false) {
	return split_on_consecutive<Predicate, OutputIterator>(n, p,
			const_cast<char*>(begin), const_cast<char*>(end), out,
			stateful_predicate);
}
...
inline std::size_t split_on_consecutive(const std::size_t &n, Predicate p,
		const unsigned char *begin, const unsigned char *end,
		OutputIterator out, const bool stateful_predicate = false) {
	return split_on_consecutive<Predicate, OutputIterator>(n, p,
			reinterpret_cast<const char*>(begin),
			reinterpret_cast<const char*>(end), out, stateful_predicate);
}
...
inline std::size_t split_on_consecutive(const std::size_t &n, Predicate p,
		const std::string &str, OutputIterator out,
		const bool stateful_predicate = false) {
	return split_on_consecutive<Predicate, OutputIterator>(n, p, str.data(),
			str.data() + str.size(), out, stateful_predicate);
}

template<typename Predicate, typename OutputIterator>
inline std::size_t split_on_consecutive_n(const std::size_t &n,
		const std::size_t &m, Predicate p, const char *begin, const char *end,
		OutputIterator out) {
	return split_on_consecutive_n<Predicate, char*, OutputIterator>(n, m, p,
			const_cast<char*>(begin), const_cast<char*>(end), out);
}

template<typename Predicate, typename OutputIterator>
inline std::size_t split_on_consecutive_n(const std::size_t &n,
		const std::size_t &m, Predicate p, const unsigned char *begin,
		const unsigned char *end, OutputIterator out) {
	return split_on_consecutive_n<Predicate, OutputIterator>(n, m, p,
			reinterpret_cast<const char*>(begin),
			reinterpret_cast<const char*>(end), out);
}
...
template<typename Predicate, typename OutputIterator>
inline std::size_t split_on_consecutive_n(const std::size_t &n,
		const std::size_t &m, Predicate p, const std::string &str,
		OutputIterator out) {
	return split_on_consecutive_n<Predicate, OutputIterator>(n, m, p,
			str.data(), str.data() + str.size(), out);
}
...
public:
	...
	inline bool operator()(InputIterator begin, InputIterator end) {
		...
		return strtk::match(s_.data(), s_.data() + s_.size(), begin, end,
...
struct convert_impl {
	...
	static bool execute(InputIterator begin, InputIterator end,
			const std::string &rem_chars, std::size_t mode, Type &t) {
		std::string s;
...
struct convert_impl<std::string> {
	template<typename InputIterator>
	static bool execute(InputIterator begin, InputIterator end,
			const std::string &rem_chars, std::size_t mode, std::string &t) {
...
public:
	...
	trim_impl(const std::size_t mode, T &t, const std::string &rem_chars = " ") :
			mode_(mode), t_(&t), rem_chars_(rem_chars) {
	}

	template<typename InputIterator>
	inline bool operator()(InputIterator begin, InputIterator end) {
		return trim_details::convert_impl<T>::execute(begin, end, rem_chars_,
				mode_, (*t_));
	}
...
public:
	...
	template<typename InputIterator>
	inline bool operator()(InputIterator begin, InputIterator end) {
		const std::size_t range_size = static_cast<std::size_t>(std::distance(
				begin, end));
		if (range_size != size_)
			return false;
...
namespace details {
static const unsigned char digit_table[] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
		0xFF, 0xFF, // 0xFF - 0x07
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, // 0x08 - 0x0F
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, // 0x10 - 0x17
		0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, // 0x18 - 0x1F
...
static const std::size_t digit_table_size = sizeof(digit_table)
...
static const unsigned char digitr[] = { "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ" };
...
struct numeric {
	enum {
		length = 0, size = 32, bound_length = 0, min_exp = 0, max_exp = 0
	};
};

template<> struct numeric<short> {
	enum {
		length = 5, size = 16, bound_length = 4
	};
};
...
template<> struct numeric<int> {
	enum {
		length = 10, size = 16, bound_length = 9
	};
};
...
template<> struct numeric<long long> {
	enum {
		length = 19, size = 24, bound_length = 18
	};
};
template<> struct numeric<unsigned long long int> {
	enum {
		length = 20, size = 24, bound_length = 19
	};
};

template<> struct numeric<float> {
	enum {
		min_exp = -37, max_exp = +38, precision = 10
	};
};
template<> struct numeric<double> {
	enum {
		min_exp = -307, max_exp = +308, precision = 15
	};
};
...
struct numeric<long double> {
	...
	enum {
		min_exp = ld::i, max_exp = ld::a, precision = ld::p
	};
};
...
template<> struct supported_conversion_to_type<
		strtk::details::conv_to_lcase_impl> {
...
template<> struct supported_conversion_to_type<
		strtk::details::conv_to_ucase_impl> {
...
inline bool string_to_type_converter_impl(Iterator &itr_external,
	...
	if (0 != length) {
		std::size_t interim_length = std::min<std::size_t>(bound_length,
				length);
		...
		if (interim_end != end) {
			if (1 == std::distance(interim_end, end)) {
				...
				static const num_type max =
						static_cast<num_type>(std::numeric_limits<T>::max());
				static const num_type penultimate_bound =
						static_cast<num_type>(max / 10);
				static const num_type final_digit = static_cast<num_type>(max
						% 10);
	...
	if (0 != length) {
		...
		if (interim_end != end) {
			if (1 == std::distance(interim_end, end)) {
				...
				static const num_type max =
						static_cast<num_type>(std::numeric_limits<T>::max());
				static const num_type min =
						static_cast<num_type>(static_cast<long long>(-1)
								* std::numeric_limits<T>::min());
				static const num_type positive_penultimate_bound =
						static_cast<num_type>(max / 10);
				static const num_type negative_penultimate_bound =
						static_cast<num_type>(min / 10);
				static const num_type positive_final_digit =
						static_cast<num_type>(max % 10);
				static const num_type negative_final_digit =
						static_cast<num_type>(min % 10);
				...
				if (digit[0] < 10) {
					if (negative) {
						if (static_cast<num_type>(t)
								> negative_penultimate_bound)
							return false;
						else if ((negative_penultimate_bound
								== static_cast<num_type>(t))
								&& (negative_final_digit < digit[0]))
							return false;
					} else {
						if (static_cast<num_type>(t)
								> positive_penultimate_bound)
							return false;
						else if ((positive_penultimate_bound
								== static_cast<num_type>(t))
...
#undef parse_digit_2
	...
	if (0 != exponent) {
		if ((std::numeric_limits<T>::max_exponent10 < (exponent + pre_decimal))
				|| (std::numeric_limits<T>::min_exponent10
						> (exponent + pre_decimal))) {
			return false;
		}

		const int e = std::abs(exponent);
		static const double fract10[] = { 0.0, 1.0E+001, 1.0E+002, 1.0E+003,
				1.0E+004, 1.0E+005, 1.0E+006, 1.0E+007, 1.0E+008, 1.0E+009,
				1.0E+010, 1.0E+011, 1.0E+012, 1.0E+013, 1.0E+014, 1.0E+015,
				1.0E+016, 1.0E+017, 1.0E+018, 1.0E+019, 1.0E+020, 1.0E+021,
				1.0E+022, 1.0E+023, 1.0E+024, 1.0E+025, 1.0E+026, 1.0E+027,
				1.0E+028, 1.0E+029, 1.0E+030, 1.0E+031, 1.0E+032, 1.0E+033,
				1.0E+034, 1.0E+035, 1.0E+036, 1.0E+037, 1.0E+038, 1.0E+039,
				1.0E+040, 1.0E+041, 1.0E+042, 1.0E+043, 1.0E+044, 1.0E+045,
				1.0E+046, 1.0E+047, 1.0E+048, 1.0E+049, 1.0E+050, 1.0E+051,
				1.0E+052, 1.0E+053, 1.0E+054, 1.0E+055, 1.0E+056, 1.0E+057,
				1.0E+058, 1.0E+059, 1.0E+060, 1.0E+061, 1.0E+062, 1.0E+063,
				1.0E+064, 1.0E+065, 1.0E+066, 1.0E+067, 1.0E+068, 1.0E+069,
				1.0E+070, 1.0E+071, 1.0E+072, 1.0E+073, 1.0E+074, 1.0E+075,
				1.0E+076, 1.0E+077, 1.0E+078, 1.0E+079, 1.0E+080, 1.0E+081,
				1.0E+082, 1.0E+083, 1.0E+084, 1.0E+085, 1.0E+086, 1.0E+087,
				1.0E+088, 1.0E+089, 1.0E+090, 1.0E+091, 1.0E+092, 1.0E+093,
				1.0E+094, 1.0E+095, 1.0E+096, 1.0E+097, 1.0E+098, 1.0E+099,
				1.0E+100, 1.0E+101, 1.0E+102, 1.0E+103, 1.0E+104, 1.0E+105,
				1.0E+106, 1.0E+107, 1.0E+108, 1.0E+109, 1.0E+110, 1.0E+111,
				1.0E+112, 1.0E+113, 1.0E+114, 1.0E+115, 1.0E+116, 1.0E+117,
				1.0E+118, 1.0E+119, 1.0E+120, 1.0E+121, 1.0E+122, 1.0E+123,
				1.0E+124, 1.0E+125, 1.0E+126, 1.0E+127, 1.0E+128, 1.0E+129,
				1.0E+130, 1.0E+131, 1.0E+132, 1.0E+133, 1.0E+134, 1.0E+135,
				1.0E+136, 1.0E+137, 1.0E+138, 1.0E+139, 1.0E+140, 1.0E+141,
				1.0E+142, 1.0E+143, 1.0E+144, 1.0E+145, 1.0E+146, 1.0E+147,
				1.0E+148, 1.0E+149, 1.0E+150, 1.0E+151, 1.0E+152, 1.0E+153,
				1.0E+154, 1.0E+155, 1.0E+156, 1.0E+157, 1.0E+158, 1.0E+159,
				1.0E+160, 1.0E+161, 1.0E+162, 1.0E+163, 1.0E+164, 1.0E+165,
				1.0E+166, 1.0E+167, 1.0E+168, 1.0E+169, 1.0E+170, 1.0E+171,
				1.0E+172, 1.0E+173, 1.0E+174, 1.0E+175, 1.0E+176, 1.0E+177,
				1.0E+178, 1.0E+179, 1.0E+180, 1.0E+181, 1.0E+182, 1.0E+183,
				1.0E+184, 1.0E+185, 1.0E+186, 1.0E+187, 1.0E+188, 1.0E+189,
				1.0E+190, 1.0E+191, 1.0E+192, 1.0E+193, 1.0E+194, 1.0E+195,
				1.0E+196, 1.0E+197, 1.0E+198, 1.0E+199, 1.0E+200, 1.0E+221,
				1.0E+222, 1.0E+223, 1.0E+224, 1.0E+225, 1.0E+226, 1.0E+227,
				1.0E+228, 1.0E+229, 1.0E+230, 1.0E+231, 1.0E+232, 1.0E+233,
				1.0E+234, 1.0E+235, 1.0E+236, 1.0E+237, 1.0E+238, 1.0E+239,
				1.0E+240, 1.0E+241, 1.0E+242, 1.0E+243, 1.0E+244, 1.0E+245,
				1.0E+246, 1.0E+247, 1.0E+248, 1.0E+249, 1.0E+250, 1.0E+251,
				1.0E+252, 1.0E+253, 1.0E+254, 1.0E+255, 1.0E+256, 1.0E+257,
				1.0E+258, 1.0E+259, 1.0E+260, 1.0E+261, 1.0E+262, 1.0E+263,
				1.0E+264, 1.0E+265, 1.0E+266, 1.0E+267, 1.0E+268, 1.0E+269,
				1.0E+270, 1.0E+271, 1.0E+272, 1.0E+273, 1.0E+274, 1.0E+275,
				1.0E+276, 1.0E+277, 1.0E+278, 1.0E+279, 1.0E+280, 1.0E+281,
				1.0E+282, 1.0E+283, 1.0E+284, 1.0E+285, 1.0E+286, 1.0E+287,
				1.0E+288, 1.0E+289, 1.0E+290, 1.0E+291, 1.0E+292, 1.0E+293,
				1.0E+294, 1.0E+295, 1.0E+296, 1.0E+297, 1.0E+298, 1.0E+299,
				1.0E+300, 1.0E+301, 1.0E+302, 1.0E+303, 1.0E+304, 1.0E+305,
				1.0E+306, 1.0E+307, 1.0E+308 };

		static const std::size_t fract10_size = sizeof(fract10)
				/ sizeof(double);
...
inline bool type_to_string_converter_impl(T value, std::string &result,
	...
	static const std::size_t buffer_size = (
			(strtk::details::numeric<T>::size < 16) ? 16 : 32);
	...
	if (value) {
		while (value >= static_cast<T>(radix_sqr)) {
			...
			memcpy(itr,
					&details::rev_3digit_lut[3 * (value - (temp_v * radix_cube))],
					3);
			value = temp_v;
		}
		...
		while (value >= static_cast<T>(radix)) {
			...
			memcpy(itr,
					&details::rev_2digit_lut[2 * (value - (temp_v * radix_sqr))],
					2);
			value = temp_v;
		}
...
template<typename Iterator>
inline bool type_to_string_converter_impl(
		const std::pair<Iterator, Iterator> &range, std::string &result,
		stdstring_range_type_tag) {
...
inline std::string type_name(const std::pair<T1, T2> &p) {
	static const std::string s = std::string(
			"std::pair<" + type_name(p.first) + "," + type_name(p.second)
					+ ">");
	return s;
}
...
inline std::string type_name(const T (&)[N]) {
	static const std::string s = details::type_name<T>() + std::string("[")
			+ type_to_string(N) + std::string("]");
	return s;
}
...
public:
	...
	inline ext_string& remove_leading(const std::string &removal_set) {
		if (removal_set.empty())
			...
			strtk::remove_leading(
					single_delimiter_predicate<std::string::value_type>(
							removal_set[0]), s_);
		else
			strtk::remove_leading(
					multiple_char_delimiter_predicate(removal_set), s_);
		return (*this);
	}
	...
	template<typename DelimiterPredicate, typename Allocator, template<typename,
			typename > class Sequence>
	inline std::size_t split_n(const DelimiterPredicate &p,
			const std::size_t &n, Sequence<std::string, Allocator> &seq,
			const split_options::type split_option = split_options::default_mode) const {
	...
	static inline ext_string all_letters() {
		static const ext_string letters(
	...
	static inline ext_string all_chars() {
		...
		strtk::iota(s.as_string().begin(), s.as_string().end(),
...
inline bool copy_file(const std::string &src_file_name,
	...
	if (remaining_bytes > 0) {
		src_file.read(&buffer[0],
				static_cast<std::streamsize>(remaining_bytes));
		dest_file.write(&buffer[0],
				static_cast<std::streamsize>(remaining_bytes));
		remaining_bytes = 0;
	}
...
inline bool concatenate(const std::string &file_name1,
		const std::string &file_name2, const std::string &output_file_name) {
	...
	while (round < 2) {
		...
		remaining_bytes = (
				(0 == round) ? file_size(file_name1) : file_size(file_name2));

		while (remaining_bytes >= block_size) {
			input_stream.read(&buffer[0],
					static_cast<std::streamsize>(block_size));
			out_file.write(&buffer[0],
					static_cast<std::streamsize>(block_size));
			remaining_bytes -= block_size;
		}
...
inline bool read_pod_proxy(std::ifstream &stream, T &t) {
	return (false
			== stream.read(reinterpret_cast<char*>(&t),
...
inline bool write_pod_proxy(std::ofstream &stream, const T &t) {
	return (false
			== stream.write(reinterpret_cast<char*>(&t),
					static_cast<std::streamsize>(sizeof(T))).fail());
}
}
...
template<typename T1, typename T2, typename T3, typename T4, typename T5,
		typename T6, typename T7, typename T8, typename T9, typename T10>
inline bool read_pod(std::ifstream &stream, T1 &t1, T2 &t2, T3 &t3, T4 &t4,
...
template<typename T1, typename T2, typename T3>
inline bool read_pod(std::ifstream &stream, T1 &t1, T2 &t2, T3 &t3) {
	return details::read_pod_proxy(stream, t1)
			&& details::read_pod_proxy(stream, t2)
			&& details::read_pod_proxy(stream, t3);
}
...
template<typename T1, typename T2>
inline bool read_pod(std::ifstream &stream, T1 &t1, T2 &t2) {
	return details::read_pod_proxy(stream, t1)
			&& details::read_pod_proxy(stream, t2);
}
...
inline bool read_pod(std::ifstream &stream, T (&t)[N]) {
	return (false
			!= stream.read(reinterpret_cast<char*>(&t[0]), sizeof(T) * N).fail());
}

template<typename T, typename Allocator,
		template<typename, typename > class Sequence>
inline bool read_pod(std::ifstream &stream, const std::size_t &count,
		Sequence<T, Allocator> &sequence) {
	T t;
...
template<typename T1, typename T2, typename T3, typename T4, typename T5,
		typename T6, typename T7, typename T8, typename T9, typename T10>
inline bool write_pod(std::ofstream &stream, const T1 &t1, const T2 &t2,
...
inline bool write_pod(std::ofstream &stream, T (&t)[N]) {
	return (false
			!= stream.write(reinterpret_cast<char*>(&t[0]), sizeof(T) * N).fail());
}
...
inline bool write_pod(std::ofstream &stream,
	...
	typename std::multiset<T, Comparator, Allocator>::iterator itr =
			multiset.begin();
	typename std::multiset<T, Comparator, Allocator>::iterator end =
			multiset.end();
	while (end != itr) {
		if (details::write_pod_proxy(stream, *itr))
			++itr;
...
inline bool read_at_offset(std::ifstream &stream, const std::size_t &offset,
		char *buffer, const std::size_t &buffer_size) {
	if (!stream)
		return false;
...
inline bool read_at_offset(const std::string &file_name,
		const std::size_t &offset, std::string &buffer,
		const std::size_t &buffer_size) {
	std::ifstream stream(file_name.c_str(), std::ios::in | std::ios::binary);
	if (!stream)
		return false;
	buffer.resize(buffer_size);
	return read_at_offset(stream, offset, const_cast<char*>(buffer.data()),
			buffer_size);
}
...
template<typename T1, typename T2, typename T3, typename T4, typename T5,
		typename T6, typename T7, typename T8, typename T9, typename T10,
		typename T11, typename T12>
inline unsigned char* read_pod(unsigned char *data, T1 &t1, T2 &t2, T3 &t3,
...
template<typename T1, typename T2, typename T3, typename T4, typename T5,
		typename T6, typename T7, typename T8, typename T9, typename T10>
inline unsigned char* read_pod(unsigned char *data, T1 &t1, T2 &t2, T3 &t3,
		T4 &t4, T5 &t5, T6 &t6, T7 &t7, T8 &t8, T9 &t9, T10 &t10) {
	t1 = (*reinterpret_cast<T1*>(data));
	data += sizeof(T1);
	t2 = (*reinterpret_cast<T2*>(data));
	data += sizeof(T2);
...
template<typename T1, typename T2>
inline unsigned char* read_pod(unsigned char *data, T1 &t1, T2 &t2) {
	t1 = (*reinterpret_cast<T1*>(data));
	data += sizeof(T1);
	t2 = (*reinterpret_cast<T2*>(data));
	data += sizeof(T2);
...
template<typename T1>
inline unsigned char* read_pod(unsigned char *data, T1 &t1) {
	t1 = (*reinterpret_cast<T1*>(data));
	data += sizeof(T1);
	return data;
}
...
template<typename T, typename Allocator,
		template<typename, typename > class Sequence>
inline unsigned char* read_pod(unsigned char *data, const std::size_t &n,
		const Sequence<T, Allocator> &sequence) {
...
template<typename T1, typename T2, typename T3, typename T4, typename T5,
		typename T6, typename T7, typename T8, typename T9, typename T10,
		typename T11, typename T12>
inline unsigned char* write_pod(unsigned char *data, const T1 &t1, const T2 &t2,
		const T3 &t3, const T4 &t4, const T5 &t5, const T6 &t6, const T7 &t7,
...
template<typename T1, typename T2>
inline unsigned char* write_pod(unsigned char *data, const T1 &t1,
		const T2 &t2) {
	(*reinterpret_cast<T1*>(data)) = t1;
	data += sizeof(T1);
	(*reinterpret_cast<T2*>(data)) = t2;
...
private:
	...
	inline bool condition_equal(const itr_type begin,
	...
	inline bool condition_within(const itr_type begin,
			const itr_type end) const {
		if (s.size() <= static_cast<std::size_t>(std::distance(begin, end))) {
...
public:
	...
	inline explicit string_condition(condition_type cond_type,
			const std::string &str) :
			cond_type_(cond_type), s(str), s_begin(
					reinterpret_cast<const unsigned char*>(s.data())), s_end(
					reinterpret_cast<const unsigned char*>(s.data() + str.size())), condition_method_(
					0) {
		switch (cond_type_) {
		case equal:
...
class prefix {
	...
	public:
		...
		explicit node(const key_value_t &key_value) :
				key_value_(key_value), value_holder_(false) {
		}

		node(const key_value_t &key_value, const value_t &v) :
				key_value_(key_value), value_holder_(true), value_(v) {
		}
...
public:
	...
	inline void insert(const key_iterator_t begin, const key_iterator_t end,
			const value_t &v) {
		if (0 == std::distance(begin, end))
			return;
	...
	template<typename key_iterator_t>
	inline bool find(const key_iterator_t begin, const key_iterator_t end,
			value_t &v) const {
...
inline void insert(prefix<std::string::const_iterator, Value> &trie,
		const std::string &key, const Value &value = Value(0)) {
	trie.insert(key.begin(), key.end(), value);
}

template<typename Value>
inline void insert(prefix<std::string::iterator, Value> &trie, const char *key,
...
public:
	...
	parameters() :
			minimum_size(1), maximum_size(
					std::numeric_limits<unsigned long long int>::max()), minimum_number_of_hashes(
					1), maximum_number_of_hashes(
					std::numeric_limits<unsigned int>::max()), projected_element_count(
					10000), false_positive_probability(
					1.0 / projected_element_count), random_seed(
	...
	virtual bool compute_optimal_parameters() {
		...
		while (k < 1000.0) {
			...
			double denominator = std::log(
					1.0 - std::pow(false_positive_probability, 1.0 / k));
...
public:
	...
	filter() :
			bit_table_(0), salt_count_(0), table_size_(0), raw_table_size_(0), projected_element_count_(
					0), inserted_element_count_(0), random_seed_(0), desired_false_positive_probability_(
					0.0) {
	}

	filter(const parameters &p) :
			bit_table_(0), projected_element_count_(p.projected_element_count), inserted_element_count_(
					0), random_seed_((p.random_seed * 0xA5A5A5A5) + 1), desired_false_positive_probability_(
	...
	inline bool operator ==(const filter &f) const {
		if (this != &f) {
			return (salt_count_ == f.salt_count_)
					...
					&& (salt_ == f.salt_)
					&& std::equal(f.bit_table_, f.bit_table_ + raw_table_size_,
							bit_table_);
		} else
			return true;
	}
	...
	inline filter& operator =(const filter &f) {
		if (this != &f) {
			...
			delete[] bit_table_;
			bit_table_ =
					new cell_type[static_cast<std::size_t>(raw_table_size_)];
	...
	inline bool write_to_file(const std::string &file_name) const {
		...
		const std::size_t buffer_size = sizeof(salt_count_)
				+ sizeof(table_size_) + sizeof(raw_table_size_)
				+ sizeof(projected_element_count_)
				+ sizeof(inserted_element_count_) + sizeof(random_seed_)
				+ sizeof(desired_false_positive_probability_)
				+ salt_count_ * sizeof(bloom_type)
				+ static_cast<std::size_t>(raw_table_size_) * sizeof(cell_type)
		...
		writer.reset(true);
		bool result = writer(salt_count_) && writer(table_size_)
				&& writer(raw_table_size_) && writer(projected_element_count_)
				&& writer(inserted_element_count_) && writer(random_seed_)
				&& writer(desired_false_positive_probability_) && writer(salt_)
	...
	inline bool read_from_file(const std::string &file_name) {
		...
		istream.close();
		bool result = reader(salt_count_) && reader(table_size_)
				&& reader(raw_table_size_) && reader(projected_element_count_)
				&& reader(inserted_element_count_) && reader(random_seed_)
				&& reader(desired_false_positive_probability_) && reader(salt_)
...
protected:
	...
	void generate_unique_salt() {
		...
		static const bloom_type predef_salt[predef_salt_count] = { 0xAAAAAAAA,
				...
				0x9F245A31, 0x83C136FC, 0xC3DA4A8C, 0xA5112C8C, 0x5271F491,
				0x9A948DAB, 0xCEE59A8D, 0xB5F525AB, 0x59D13217, 0x24E7C331,
				0x697C2103, 0x84B0A460, 0x86156DA9, 0xAEF2AC68, 0x23243DA5,
				0x3F649643, 0x5FA495A8, 0x67710DF8, 0x9A6C499E, 0xDCFB0227,
				0x46A43433, 0x1832B07A, 0xC46AFF3C, 0xB9C8FFF0, 0xC9500467,
				0x34431BDF, 0xB652432B, 0xE367F12B, 0x427F4C1B, 0x224C006E,
				0x2E7E5A89, 0x96F99AA5, 0x0BEB452A, 0x2FD87C39, 0x74B2E1FB,
				0x222EFD24, 0xF357F60C, 0x440FCB1E, 0x8BBE030F, 0x6704DC29,
				0x1144D12F, 0x948B1355, 0x6D8FD7E9, 0x1C11A014, 0xADD1592F,
				0xFB3C712E, 0xFC77642F, 0xF9C4CE8C, 0x31312FB9, 0x08B0DD79,
				0x318FA6E7, 0xC040D23D, 0xC0589AA7, 0x0CA5C075, 0xF874B172,
				0x0CF914D5, 0x784D3280, 0x4E8CFEBC, 0xC569F575, 0xCDB2A091,
		...
		if (salt_count_ <= predef_salt_count) {
			...
			while (salt_.size() < salt_count_) {
				...
				if (0 == current_salt)
					continue;
				if (salt_.end()
						== std::find(salt_.begin(), salt_.end(),
								current_salt)) {
...
public:
	...
	inline bool compress(const double &percentage) {
		...
		unsigned long long int new_table_size =
				static_cast<unsigned long long int>((size_list.back()
...
inline unsigned int hash(const std::string &s, unsigned int seed =
		details::hash_seed) {
	unsigned int hash_value = seed;
...
public:
	...
	scoped_restore(T &t, const bool restore = true) :
			restore_(restore), reference_(t), copy_(t) {
	}
...
public:
	...
	inline bool operator==(const semantic_action_impl &sa) const {
		return (0 != function_holder_) && (0 != sa.function_holder_)
	...
	template<typename InputIterator>
	inline bool operator()(
			const std::pair<InputIterator, InputIterator> &r) const {
	...
	inline void assign(Function &f) {
		static const std::size_t type_size =
				sizeof(function_holder<Function>(f));
		function_holder_ = construct<Function,
				type_size <= function_holder_buffer_size>::type(f,
...
private:
	...
	struct construct<Function, true> {
		inline static function_holder_ptr type(Function &f,
	...
	enum {
		function_holder_buffer_size = 64
	};
...
template<typename T>
inline bool type_to_string_converter_impl(
		const strtk::util::attribute<T> &attrib, std::string &result,
		attribute_type_tag) {
...
template<> struct supported_conversion_to_type<strtk::util::semantic_action_impl> {
	typedef semantic_action_type_tag type;
};
template<> struct supported_conversion_from_type<
...
inline bool type_to_string_converter_impl(
		const strtk::util::semantic_action_impl&, std::string &result,
...
public:
	...
	inline bool operator==(const value &v) const {
		return (0 != type_holder_) && (0 != v.type_holder_)
		...
		if (&v != this) {
			if (0 != v.type_holder_) {
				std::copy(v.type_holder_buffer_,
						...
						type_holder_buffer_);
				type_holder_ =
						reinterpret_cast<type_holder_base*>(type_holder_buffer_);
			}
		}
		return *this;
	}
	...
	inline void assign(T &t) {
		...
		type_holder_ = construct<T, type_size <= type_holder_buffer_size>::type(
				t, type_holder_buffer_);
	}
...
private:
	...
	enum {
		type_holder_buffer_size = 2
				* sizeof(type_holder<unsigned long long int> )
	};
	unsigned char type_holder_buffer_[type_holder_buffer_size];
};

template<typename Key, typename T, typename Comparator, typename MapAllocator,
		typename OutputIterator>
...
template<typename Key, typename T, typename Comparator, typename MapAllocator,
		typename SequenceAllocator, template<typename, typename > class Sequence>
inline void make_key_list(const std::map<Key, T, Comparator, MapAllocator> &map,
...
template<typename Key, typename T, typename Comparator, typename MapAllocator,
		typename OutputIterator>
inline void make_value_list(
		const std::multimap<Key, T, Comparator, MapAllocator> &map,
		const Key &key, OutputIterator out) {
	if (map.empty())
		return;
...
inline void make_value_list(
		const std::multimap<Key, T, Comparator, MapAllocator> &map,
		const Key &key, Sequence<T, SequenceAllocator> &sequence) {
...
inline void delete_all(std::map<Key, T*, Comparator, Allocator> &cont) {
	typename std::map<Key, T*, Comparator, Allocator>::iterator itr =
			cont.begin();
	typename std::map<Key, T*, Comparator, Allocator>::iterator end =
			cont.end();
	while (end != itr) {
		delete (*itr).second;
		++itr;
...
template<typename Predicate, typename Key, typename T, typename Comparator,
		typename Allocator>
...
template<typename Predicate, typename T, typename Comparator, typename Allocator>
inline void delete_if(const Predicate &p,
...
template<typename T, typename Allocator,
		template<typename, typename > class Sequence>
inline void push_back(Sequence<T, Allocator> &sequence, const T &v1,
		const T &v2, const T &v3, const T &v4, const T &v5, const T &v6,
		const T &v7, const T &v8, const T &v9, const T &v10, const T &v11) {
...
inline void push_back(Sequence<T, Allocator> &sequence, const T &v1,
		const T &v2, const T &v3, const T &v4, const T &v5, const T &v6,
		const T &v7, const T &v8, const T &v9, const T &v10) {
	sequence.push_back(v1);
	sequence.push_back(v2);
	sequence.push_back(v3);
	sequence.push_back(v4);
...
template<typename T, typename Allocator,
		template<typename, typename > class Sequence>
inline void push_back(Sequence<T, Allocator> &sequence, const T &v1,
		const T &v2, const T &v3, const T &v4, const T &v5) {
	sequence.push_back(v1);
	sequence.push_back(v2);
	sequence.push_back(v3);
	sequence.push_back(v4);
...
template<typename T, typename Allocator,
		template<typename, typename > class Sequence>
inline void push_back(Sequence<T, Allocator> &sequence, const T &v1,
		const T &v2, const T &v3, const T &v4) {
	sequence.push_back(v1);
	sequence.push_back(v2);
	sequence.push_back(v3);
	sequence.push_back(v4);
...
template<typename T, typename Comparator, typename Allocator>
inline void push_back(std::set<T, Comparator, Allocator> &set, const T &v1,
		const T &v2, const T &v3, const T &v4, const T &v5, const T &v6,
		const T &v7, const T &v8, const T &v9, const T &v10) {
	set.insert(v1);
	set.insert(v2);
...
template<typename T, typename Comparator, typename Allocator>
inline void push_back(std::multiset<T, Comparator, Allocator> &set, const T &v1,
		const T &v2, const T &v3, const T &v4, const T &v5, const T &v6,
		const T &v7, const T &v8, const T &v9, const T &v10) {
	set.insert(v1);
	set.insert(v2);
...
public:
	...
	column_selector_base(const column_list_t &column_list) :
			column_list_(column_list), current_index_(0), target_index_(
					column_list_.index_list[0]), col_list_index_(0), error_count_(
					0) {
	}
...
protected:
	...
	inline void process(const std::pair<Iterator, Iterator> &r) {
		if (current_index_ > target_index_)
			...
			typename colsel_value_list::value_t &v =
					cvl_.value_list[col_list_index_];
...
class column_selector_impl: public column_selector_base<
		column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11>,
		12> {
public:

	typedef column_selector_base<
			column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10,
					T11>, 12> csb_t;
	typedef column_list_impl<12> column_list_t;

	column_selector_impl(const column_list_t &column_list, T0 &t0, T1 &t1,
...
template<typename T0, typename T1, typename T2, typename T3, typename T4,
		...
		typename T10>
class column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10> : public column_selector_base<
		column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9, T10>, 11> {
public:
...
template<typename T0, typename T1, typename T2, typename T3, typename T4,
		typename T5, typename T6, typename T7, typename T8, typename T9>
class column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9> : public column_selector_base<
		column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>, 10> {
public:

	typedef column_selector_base<
			column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>, 10> csb_t;
...
class column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8> : public column_selector_base<
		column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8>, 9> {
public:

	typedef column_selector_base<
			column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8>, 9> csb_t;
...
class column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7> : public column_selector_base<
		column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7>, 8> {
public:

	typedef column_selector_base<
			column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7>, 8> csb_t;
...
class column_selector_impl<T0, T1, T2, T3, T4, T5, T6> : public column_selector_base<
		column_selector_impl<T0, T1, T2, T3, T4, T5, T6>, 7> {
public:

	typedef column_selector_base<
			column_selector_impl<T0, T1, T2, T3, T4, T5, T6>, 7> csb_t;
...
template<typename T0, typename T1, typename T2, typename T3, typename T4,
		typename T5>
class column_selector_impl<T0, T1, T2, T3, T4, T5> : public column_selector_base<
		column_selector_impl<T0, T1, T2, T3, T4, T5>, 6> {
public:
...
template<typename T0, typename T1, typename T2, typename T3, typename T4>
class column_selector_impl<T0, T1, T2, T3, T4> : public column_selector_base<
		column_selector_impl<T0, T1, T2, T3, T4>, 5> {
public:
...
template<typename T0, typename T1, typename T2, typename T3>
class column_selector_impl<T0, T1, T2, T3> : public column_selector_base<
		column_selector_impl<T0, T1, T2, T3>, 4> {
public:
...
template<typename T0, typename T1, typename T2>
class column_selector_impl<T0, T1, T2> : public column_selector_base<
		column_selector_impl<T0, T1, T2>, 3> {
public:
...
template<typename T0, typename T1>
class column_selector_impl<T0, T1> : public column_selector_base<
		column_selector_impl<T0, T1>, 2> {
public:
...
template<typename T0>
class column_selector_impl<T0> : public column_selector_base<
		column_selector_impl<T0>, 1> {
public:
...
inline details::column_list_impl<12> column_list(const std::size_t &idx0,
		const std::size_t &idx1, const std::size_t &idx2,
		const std::size_t &idx3, const std::size_t &idx4,
		const std::size_t &idx5, const std::size_t &idx6,
		const std::size_t &idx7, const std::size_t &idx8,
		const std::size_t &idx9, const std::size_t &idx10,
...
inline details::column_list_impl<11> column_list(const std::size_t &idx0,
		const std::size_t &idx1, const std::size_t &idx2,
		const std::size_t &idx3, const std::size_t &idx4,
		const std::size_t &idx5, const std::size_t &idx6,
		const std::size_t &idx7, const std::size_t &idx8,
...
inline details::column_list_impl<10> column_list(const std::size_t &idx0,
		const std::size_t &idx1, const std::size_t &idx2,
		const std::size_t &idx3, const std::size_t &idx4,
		const std::size_t &idx5, const std::size_t &idx6,
		const std::size_t &idx7, const std::size_t &idx8,
...
inline details::column_list_impl<9> column_list(const std::size_t &idx0,
		const std::size_t &idx1, const std::size_t &idx2,
		const std::size_t &idx3, const std::size_t &idx4,
		const std::size_t &idx5, const std::size_t &idx6,
		const std::size_t &idx7, const std::size_t &idx8) {
...
inline details::column_list_impl<8> column_list(const std::size_t &idx0,
		const std::size_t &idx1, const std::size_t &idx2,
		const std::size_t &idx3, const std::size_t &idx4,
		const std::size_t &idx5, const std::size_t &idx6,
...
inline details::column_list_impl<7> column_list(const std::size_t &idx0,
		const std::size_t &idx1, const std::size_t &idx2,
		const std::size_t &idx3, const std::size_t &idx4,
		const std::size_t &idx5, const std::size_t &idx6) {
...
inline details::column_list_impl<6> column_list(const std::size_t &idx0,
		const std::size_t &idx1, const std::size_t &idx2,
		const std::size_t &idx3, const std::size_t &idx4,
...
inline details::column_list_impl<5> column_list(const std::size_t &idx0,
		const std::size_t &idx1, const std::size_t &idx2,
		const std::size_t &idx3, const std::size_t &idx4) {
...
inline details::column_list_impl<4> column_list(const std::size_t &idx0,
		const std::size_t &idx1, const std::size_t &idx2,
...
inline details::column_list_impl<3> column_list(const std::size_t &idx0,
		const std::size_t &idx1, const std::size_t &idx2) {
...
inline details::column_list_impl<2> column_list(const std::size_t &idx0,
...
inline details::column_list_impl<1> column_list(const std::size_t &idx0) {
...
inline typename details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7,
		T8, T9> column_selector(const details::column_list_impl<11> &col_list,
		T0 &t0, T1 &t1, T2 &t2, T3 &t3, T4 &t4, T5 &t5, T6 &t6, T7 &t7, T8 &t8,
		T9 &t9, T10 &t10, T11 &t11) {
	return details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9,
			T10, T11>(col_list, t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10,
			t11);
}
...
inline typename details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7,
		...
		T9 &t9, T10 &t10) {
	return details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9,
			T10>(col_list, t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10);
}

template<typename T0, typename T1, typename T2, typename T3, typename T4,
		typename T5, typename T6, typename T7, typename T8, typename T9>
inline typename details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7,
		T8, T9> column_selector(const details::column_list_impl<10> &col_list,
		T0 &t0, T1 &t1, T2 &t2, T3 &t3, T4 &t4, T5 &t5, T6 &t6, T7 &t7, T8 &t8,
		T9 &t9) {
	return details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>(
...
template<typename T0, typename T1, typename T2, typename T3, typename T4,
		typename T5, typename T6, typename T7, typename T8>
inline typename details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8> column_selector(
		const details::column_list_impl<9> &col_list, T0 &t0, T1 &t1, T2 &t2,
		T3 &t3, T4 &t4, T5 &t5, T6 &t6, T7 &t7, T8 &t8) {
	return details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7, T8>(
			col_list, t0, t1, t2, t3, t4, t5, t6, t7, t8);
}

template<typename T0, typename T1, typename T2, typename T3, typename T4,
		typename T5, typename T6, typename T7>
inline typename details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7> column_selector(
		const details::column_list_impl<8> &col_list, T0 &t0, T1 &t1, T2 &t2,
		T3 &t3, T4 &t4, T5 &t5, T6 &t6, T7 &t7) {
	return details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6, T7>(
			col_list, t0, t1, t2, t3, t4, t5, t6, t7);
}

template<typename T0, typename T1, typename T2, typename T3, typename T4,
		typename T5, typename T6>
inline typename details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6> column_selector(
		const details::column_list_impl<7> &col_list, T0 &t0, T1 &t1, T2 &t2,
		T3 &t3, T4 &t4, T5 &t5, T6 &t6) {
	return details::column_selector_impl<T0, T1, T2, T3, T4, T5, T6>(col_list,
			t0, t1, t2, t3, t4, t5, t6);
}

template<typename T0, typename T1, typename T2, typename T3, typename T4,
		typename T5>
inline typename details::column_selector_impl<T0, T1, T2, T3, T4, T5> column_selector(
		const details::column_list_impl<6> &col_list, T0 &t0, T1 &t1, T2 &t2,
		T3 &t3, T4 &t4, T5 &t5) {
	return details::column_selector_impl<T0, T1, T2, T3, T4, T5>(col_list, t0,
			t1, t2, t3, t4, t5);
}

template<typename T0, typename T1, typename T2, typename T3, typename T4>
inline typename details::column_selector_impl<T0, T1, T2, T3, T4> column_selector(
		const details::column_list_impl<5> &col_list, T0 &t0, T1 &t1, T2 &t2,
		T3 &t3, T4 &t4) {
	return details::column_selector_impl<T0, T1, T2, T3, T4>(col_list, t0, t1,
			t2, t3, t4);
}

template<typename T0, typename T1, typename T2, typename T3>
inline typename details::column_selector_impl<T0, T1, T2, T3> column_selector(
		const details::column_list_impl<4> &col_list, T0 &t0, T1 &t1, T2 &t2,
		T3 &t3) {
	return details::column_selector_impl<T0, T1, T2, T3>(col_list, t0, t1, t2,
			t3);
}

template<typename T0, typename T1, typename T2>
inline typename details::column_selector_impl<T0, T1, T2> column_selector(
		const details::column_list_impl<3> &col_list, T0 &t0, T1 &t1, T2 &t2) {
	return details::column_selector_impl<T0, T1, T2>(col_list, t0, t1, t2);
}

template<typename T0, typename T1>
inline typename details::column_selector_impl<T0, T1> column_selector(
		const details::column_list_impl<2> &col_list, T0 &t0, T1 &t1) {
	return details::column_selector_impl<T0, T1>(col_list, t0, t1);
}

template<typename T0>
inline typename details::column_selector_impl<T0> column_selector(
		const details::column_list_impl<1> &col_list, T0 &t0) {
	return details::column_selector_impl<T0>(col_list, t0);
}
...
template<std::size_t N, typename T, typename Allocator, template<typename,
		typename > class Sequence>
inline typename details::compose_st_selector_impl<T, N>::type column_selector(
		const details::column_list_impl<N> &col_list,
		Sequence<T, Allocator> &seq) {
	if (seq.size() >= N) {
...
public:
	...
	column_selector_iterator_impl(
			const details::column_list_impl<N> &column_list,
			iterator_type (&token_list)[N]) :
			column_list_(column_list), token_list_(token_list), current_index_(
					0), target_index_(column_list_.index_list[0]), col_list_index_(
					0) {
	}
...
template<typename InputIterator, typename T0, typename T1, typename T2,
		typename T3, typename T4, typename T5, typename T6, typename T7,
		typename T8, typename T9, typename T10, typename T11>
inline bool parse_columns(const InputIterator begin, const InputIterator end,
		const std::string &delimiters,
		const details::column_list_impl<12> &column_list, T0 &t0, T1 &t1,
		T2 &t2, T3 &t3, T4 &t4, T5 &t5, T6 &t6, T7 &t7, T8 &t8, T9 &t9,
		T10 &t10, T11 &t11) {
	strtk_parse_columns_impl(12)
	strtk_parse_col_token(0)
	strtk_parse_col_token(1)
	strtk_parse_col_token(2)
	strtk_parse_col_token(3)
	strtk_parse_col_token(4)
	strtk_parse_col_token(5)
	strtk_parse_col_token(6)
	strtk_parse_col_token(7)
	strtk_parse_col_token(8)
	strtk_parse_col_token(9)
	strtk_parse_col_token(10)
	strtk_parse_col_token(11)
	return true;
}
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
		const std::string &delimiters,
		const details::column_list_impl<11> &column_list, T0 &t0, T1 &t1,
		T2 &t2, T3 &t3, T4 &t4, T5 &t5, T6 &t6, T7 &t7, T8 &t8, T9 &t9,
		T10 &t10) {
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
		const std::string &delimiters,
		const details::column_list_impl<10> &column_list, T0 &t0, T1 &t1,
		T2 &t2, T3 &t3, T4 &t4, T5 &t5, T6 &t6, T7 &t7, T8 &t8, T9 &t9) {
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
		const std::string &delimiters,
		const details::column_list_impl<9> &column_list, T0 &t0, T1 &t1, T2 &t2,
		T3 &t3, T4 &t4, T5 &t5, T6 &t6, T7 &t7, T8 &t8) {
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
		const std::string &delimiters,
		const details::column_list_impl<8> &column_list, T0 &t0, T1 &t1, T2 &t2,
		T3 &t3, T4 &t4, T5 &t5, T6 &t6, T7 &t7) {
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
		const std::string &delimiters,
		const details::column_list_impl<7> &column_list, T0 &t0, T1 &t1, T2 &t2,
		T3 &t3, T4 &t4, T5 &t5, T6 &t6) {
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
		const std::string &delimiters,
		const details::column_list_impl<6> &column_list, T0 &t0, T1 &t1, T2 &t2,
		T3 &t3, T4 &t4, T5 &t5) {
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
		const std::string &delimiters,
		const details::column_list_impl<5> &column_list, T0 &t0, T1 &t1, T2 &t2,
		T3 &t3, T4 &t4) {
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
		const std::string &delimiters,
		const details::column_list_impl<4> &column_list, T0 &t0, T1 &t1, T2 &t2,
		T3 &t3) {
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
		const std::string &delimiters,
		const details::column_list_impl<3> &column_list, T0 &t0, T1 &t1,
		T2 &t2) {
...
template<typename InputIterator, typename T0, typename T1>
inline bool parse_columns(const InputIterator begin, const InputIterator end,
		const std::string &delimiters,
		const details::column_list_impl<2> &column_list, T0 &t0, T1 &t1) {
...
template<typename InputIterator, typename T0>
inline bool parse_columns(const InputIterator begin, const InputIterator end,
		const std::string &delimiters,
		const details::column_list_impl<1> &column_list, T0 &t0) {
...
inline bool parse_columns(const InputIterator begin, const InputIterator end,
	...
	strtk_parse_col_token_seq(1)
	strtk_parse_col_token_seq(2)
	strtk_parse_col_token_seq(3)
	strtk_parse_col_token_seq(4)
	strtk_parse_col_token_seq(5)
	strtk_parse_col_token_seq(6)
	strtk_parse_col_token_seq(7)
	strtk_parse_col_token_seq(8)
	strtk_parse_col_token_seq(9)
	strtk_parse_col_token_seq(10)
	strtk_parse_col_token_seq(11)
	return true;
}
...
inline bool parse_columns(const std::string &data,
		const std::string &delimiters,
		const details::column_list_impl<1> &column_list, T &t) {
	strtk_parse_col_begin()
	t
	strtk_parse_col_end()
}
...
template<typename Iterator1, typename Iterator2>
inline std::pair<Iterator1, Iterator2> make_pair(
...
template<typename Iterator>
inline std::pair<Iterator, Iterator> make_pair(
		const std::pair<const char*, const char*> &p) {
...
template<std::size_t N>
inline bool set_array(unsigned char (&a)[N], const std::string &s,
		const bool pad = false, const unsigned char padding = '0') {
	if (N < s.size())
		return false;
...
inline bool set_array(unsigned char (&dest)[N], unsigned char (&src)[M],
		const bool pad = false, const unsigned char padding = '0') {
	if (N < M)
		return false;
...
struct options {
	...
	options() :
			pair_block_delimiter(0), pair_delimiter(0) {
	}
...
public:
	...
	parser(const Options &opts) :
			options_(opts), kv_map_(opts), pair_block_sdp_(
					options_.pair_block_delimiter), pair_delimiter_sdp_(
					options_.pair_delimiter) {
	...
	inline bool operator()(const range_type &data, const bool ignore_failures =
			false) {
		if (!ignore_failures) {
			pair_list_.clear();
			const std::size_t pair_count = split(pair_block_sdp_, data.first,
					data.second, std::back_inserter(pair_list_));
			...
			for (std::size_t i = 0; i < pair_count; ++i) {
				...
				if (0 == std::distance(r.first, r.second))
					continue;
				else if (!split_pair(r.first, r.second, pair_delimiter_sdp_,
			...
			split(pair_block_sdp_, data.first, data.second,
					strtk::functional_inserter(processor));
			return true;
		}
	}

	inline bool operator()(const std::string &s, const bool ignore_failures =
			false) {
...
private:
	...
	public:
		...
		inline void operator()(const range_type &r) {
			if (r.first == r.second)
				return;
			if (split_pair(r.first, r.second, parser_.pair_delimiter_sdp_,
					key_range, value_range)) {
...
struct keygen<Range, unsigned int> {
	static inline unsigned int transform(const Range &key_range) {
		...
		if (strtk::fast::numeric_convert(
				std::distance(key_range.first, key_range.second),
				key_range.first, result, true))
			return result;
		else
...
template<typename KeyType, typename MapType = std::map<KeyType,
		strtk::util::value>, typename KeyValidator = details::no_op_validator,
...
static inline std::ostream& operator<<(std::ostream &os,
		const strtk::std_string::tokenizer<
				strtk::single_delimiter_predicate<char> >::type::iterator &range) {
...
static inline std::ostream& operator<<(std::ostream &os,
		const strtk::std_string::tokenizer<
				strtk::single_delimiter_predicate<unsigned char> >::type::iterator &range) {
	os << std::string((*range).first, (*range).second);
	return os;
}
...
static inline std::ostream& operator<<(std::ostream &os,
		const strtk::std_string::tokenizer<
				strtk::multiple_char_delimiter_predicate>::type::iterator &range) {
	os << std::string((*range).first, (*range).second);
	return os;
}
...
static inline std::string data() {
	static const std::string info_str = std::string(library) + std::string(" v")
