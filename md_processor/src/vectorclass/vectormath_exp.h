...
// Pad� expansion uses less code and fewer registers, but is slower
template<class VTYPE, class BTYPE, int M1> 
static inline VTYPE exp_d(VTYPE const & initial_x) {
...
// The logarithm is calculated by Pad� approximation with 6'th degree 
...
static inline VTYPE pow_template_d(VTYPE const & x0, VTYPE const & y) {
    ...
    // coefficients for Pad� polynomials
    ...
    // add initial terms to Pad� expansion
    lg = (x - 0.5 * x2) + lg1;
    ...
    z = select(xfinite, z, select(y == 0., VTYPE(1.), select(y < 0., VTYPE(0.), infinite_vec<VTYPE>() | (yodd & x0))));
...
static inline VTYPE pow_template_f(VTYPE const & x0, VTYPE const & y) {
    ...
    z = select(xfinite, z, select(y == 0.f, VTYPE(1.f), select(y < 0.f, VTYPE(0.f), infinite_vec<VTYPE>() | (yodd & x0))));
    z = select(is_nan(x0), select(is_nan(y), x0 | y, x0), select(is_nan(y), y, z));
    return z;
};
