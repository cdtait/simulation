 ...
 */
static PrintType char2printtype(char print_type) {
	if (print_type == 'T') {
...
void print_usage() {
	std::string message =
{R"(Usage: md_processor [ [ -f <file name> -a [F|M] ] | [ZR|ZP|ZS] ] [-p T|C] [-d M|H|V] [-x L] [-t A|S|C]  -s [ [P|D|N ] | [U --publish_address=<address>] ]                   
       ...
       -a select the adapter to source the market data
         ...
         M is memory mapped file based input
...
md(int argc, char **argv) {
	...
	// Start the test by configuring the options then passing it on to run
	select_publisher_and_run(file_name,adapter,data_struct,parser,tokenizer,publisher,print_type,args);
}
...
int run(MD &md, Adapter &adapter, PrintType printType,const arguments& args)
{
...
void select_adapter_and_run(const std::string& file_name,
	...
	if (adapter == "F") {
		...
		run(md,adapter,printType,args);
	} else if (adapter == "M") {
		memorymapped_file_adapter<TokenContainer,MD> adapter(file_name.c_str());
		run(md,adapter,printType,args);
	} else if (adapter == "ZR") {
...
void select_data_structure_and_run(const std::string& file_name,
		const std::string & adapter,
	...
	if (data_struct == "M") {
		...
		select_adapter_and_run<TokenContainer>(file_name, md_handler, adapter, print_type,args);
 ...
 */
template <typename TokenContainer,typename Publisher>
void select_parser_and_run(const std::string& file_name,
		const std::string adapter,
		const std::string data_struct,
		...
		const arguments& args) {
	if (parser == "L") {
		select_data_structure_and_run<TokenContainer,Publisher>(file_name,adapter,data_struct,print_type,args);
	}
...
void select_token_method_and_run(const std::string& file_name,
		const std::string& adapter,
		const std::string&  data_struct,
		const std::string&  parser,
	...
	if (tokenizer=="A") {
		...
		select_parser_and_run<json_token_array,Publisher>(file_name,adapter,data_struct,parser,print_type,args);
	}
	else if(tokenizer=="S") {
		// Tokenized csv std::string vector
		select_parser_and_run<string_token_vector,Publisher>(file_name,adapter,data_struct,parser,print_type,args);
	}
	else if(tokenizer=="C") {
		// Tokenized csv char * vector
		select_parser_and_run<char_token_vector,Publisher>(file_name,adapter,data_struct,parser,print_type,args);
	}
	else {
		print_usage();
		exit(EXIT_FAILURE);
 ...
 * @param print_type type print format type
 */
void select_publisher_and_run(const std::string& file_name,
		const std::string& adapter,
		const std::string&  data_struct,
		const std::string&  parser,
		const std::string&  tokenizer,
	...
	if (publisher=="D") {
		// Tokenized simple csv json array
		select_token_method_and_run<disruptor_publisher<>>(file_name,adapter,data_struct,parser,tokenizer,print_type,args);
	}
	else if(publisher=="U") {
		// Tokenized csv char * vector
		select_token_method_and_run<udp_publisher<>>(file_name,adapter,data_struct,parser,tokenizer,print_type,args);
	}
	else if(publisher=="P") {
		// Tokenized csv std::string vector
		select_token_method_and_run<print_publisher<>>(file_name,adapter,data_struct,parser,tokenizer,print_type,args);
	}
	else if(publisher=="N") {
		// Tokenized csv char * vector
		select_token_method_and_run<null_publisher>(file_name,adapter,data_struct,parser,tokenizer,print_type,args);
	}
	else {
		print_usage();
		exit(EXIT_FAILURE);
